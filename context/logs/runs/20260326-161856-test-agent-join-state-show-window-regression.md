...
## Injected Trace Root Cause

A direct injected trace around `resume_runtime_session()` confirmed the overwrite path:

- resumed `HeadlessSessionState` did not contain any `tmux_window_name`
- the subsequent `persist_manifest()` rewrite emitted:
  - `tmux.primary_window_name = null`
  - `interactive.tmux_window_name = null`
  - `backend_state.tmux_window_name = null`
- the same write still preserved `launch_plan.metadata.tmux_window_name = "claude"`

The live source tree being imported by `houmao-mgr` is still missing the required preservation hooks:

- `src/houmao/agents/realm_controller/backends/headless_base.py`
  - `HeadlessSessionState` has no `tmux_window_name`
  - `headless_backend_state_payload()` does not serialize `tmux_window_name`
- `src/houmao/agents/realm_controller/runtime.py`
  - `_resume_local_interactive_state()` does not restore `tmux_window_name` from persisted manifest fields or joined launch metadata
- `src/houmao/srv_ctrl/commands/managed_agents.py`
  - `_managed_tmux_window_name_from_manifest_path()` has no joined-launch-metadata fallback and therefore returns the default `agent`

So the reproduced failure is not a new regression after the fix; the real CLI path is still running source that lacks the intended fix in those exact code seams.
