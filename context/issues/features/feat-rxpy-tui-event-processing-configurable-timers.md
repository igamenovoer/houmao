...
- fixed short read windows in Codex app-server event loops (`0.25s`) and interrupt timeout (`3.0s`) in `src/gig_agents/agents/brain_launch_runtime/backends/codex_app_server.py`
- default CAO polling/time budget values (`poll_interval_seconds=0.4`, `timeout_seconds=120.0`) in `src/gig_agents/agents/brain_launch_runtime/backends/cao_rest.py`
- headless tmux completion polling defaults in `src/gig_agents/agents/brain_launch_runtime/backends/headless_runner.py`
