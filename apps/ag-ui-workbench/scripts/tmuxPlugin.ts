...
const TMUX_PREFIX = "/__houmao_tmux";
...
async function handleTmuxHttpRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
  const requestUrl = new URL(req.url ?? "/", "http://127.0.0.1");
...
function handleTmuxAttachSocket(ws: WebSocket): void {
  if (tmuxFixtureEnabled()) {
