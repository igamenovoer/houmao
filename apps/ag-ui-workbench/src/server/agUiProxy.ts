...
export const PROXY_PREFIX = "/__houmao_ag_ui_proxy";
...
export async function handleProxyRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  try {
    ...
    await pipeFetchResponseToServerResponse(upstream, res);
  } catch (error) {
...
export function forwardedRequestHeaders(req: IncomingMessage): Headers {
...
export function resolveAllowedTarget(value: string | null): TargetPolicyResult {
  if (!value) {
    return {
      allowed: false,
      status: 400,
...
export async function pipeFetchResponseToServerResponse(
  upstream: Response,
  res: ServerResponse,
): Promise<void> {
