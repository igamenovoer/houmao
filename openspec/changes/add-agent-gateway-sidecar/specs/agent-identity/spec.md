...
When a caller resolves a non-path-like `--agent-identity` value to a live tmux-backed session, gateway-aware tools SHALL be able to discover gateway pointers from that tmux session environment when those pointers are present.
...
- **AND WHEN** `AGENTSYS_GATEWAY_ROOT` and related gateway pointers are absent from that tmux session environment
...
- **AND WHEN** the tmux session environment is missing or contains invalid required gateway discovery pointers
