...
- **AND WHEN** the effective Gemini launch environment does not already select API-key or Vertex auth
...
#### Scenario: Explicit non-OAuth Gemini auth selection is preserved
- **WHEN** the effective Gemini launch environment explicitly selects a different supported auth mode such as API-key or Vertex auth
