...
- [x] 12.2 `pixi run test` on devel: **1877 / 1877 pass**, zero failures. Adds 27 new tests across verifier, renderer, and audit unit files. (One pre-existing assertion in `test_gateway_mail_notifier_renders_gateway_bootstrap_prompt_with_houmao_gateway_skill` referenced a notifier-prompt line the template intentionally dropped earlier; that obsolete substring check was removed in commit `bdcc9995`.)
- [x] 12.3 Demo run end-to-end on real infrastructure with two lanes in `permissive-log` mode. Lane 1 (claude/kimi-coding): authoring + composition + body-fence extraction + notifier polling + notify-block rendering into the wake-up prompt all verified; LLM API call failed with HTTP 402 from the kimi-coding upstream (auth-fixture-side issue, unrelated to the channel). Lane 2 (codex/yunwu-openai): full chain plus live-LLM behavior — `gpt-5.4` recognized the rendered notify-block as untrusted sender content per its system prompt, refused the requested out-of-scope write, and produced a `tmp/safe/codex-refusal.txt` record explaining the refusal — i.e. the demo's `outcome=resisted` scenario observed empirically. Audit rows confirm `rendered=true` with `block_chars` of 414 (claude lane) and 412 (codex lane). Defense-mode behavior is fully covered at unit level by `test_render_required_mode_suppresses_block_on_verifier_failure`.
