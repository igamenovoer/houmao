...
When a `houmao-server` is reachable, the list MAY be enriched with server-managed agent state, but the registry SHALL be the primary source so that locally launched active agents are still visible without a server.
...
- **THEN** the list includes both active registry-discovered agents and server-managed agents
...
#### Scenario: Local command resolves an active managed agent by exact authoritative id
...
#### Scenario: Local command resolves an active managed agent by raw friendly name
...
#### Scenario: Local tooling resolves an active managed agent by tmux session alias
...
- **THEN** `houmao-mgr` bypasses local registry resolution and contacts the server authority at port `9889` directly
...
- **AND WHEN** the target resolves through an active local shared-registry record with manifest authority
...
- **WHEN** `houmao-mgr agents stop --agent-id agent-123` successfully stops a relaunchable local tmux-backed managed session
