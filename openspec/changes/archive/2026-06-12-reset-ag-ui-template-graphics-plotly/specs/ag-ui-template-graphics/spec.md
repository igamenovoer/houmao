...
**Reason**: Layer 1 now has one renderer id, `plotly`, so fallback semantics would preserve an obsolete multi-backend contract.

**Migration**: Agents SHALL omit renderer fallback lists or set `renderer.preferred` to `plotly`.

### Requirement: Workbench renders template graphics through Recharts and Vega-Lite
**Reason**: Recharts and Vega-Lite are no longer Layer 1 template renderers. Plotly.js is the only Layer 1 renderer, and Vega-Lite is reserved for a separate Layer 2 DSL graphics capability.

**Migration**: Workbench code, fixtures, docs, and tests SHALL render `houmao.graphic.template` payloads through Plotly.js and remove Layer 1 Recharts or Vega-Lite renderer selection.
