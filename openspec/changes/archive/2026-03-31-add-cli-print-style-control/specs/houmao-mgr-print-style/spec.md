...
- `json`: Machine-readable JSON with 2-space indent and sorted keys. Equivalent to current `emit_json()` behavior.
- `fancy`: Rich-formatted output using the `rich` library — tables, panels, and colored status indicators.
...
#### Scenario: JSON output matches previous default behavior
- **WHEN** an operator runs `houmao-mgr agents list --print-json`
- **THEN** the output is valid JSON with 2-space indent and sorted keys
- **AND THEN** the output is identical in structure to the pre-change `emit_json()` output
...
### Requirement: `emit()` replaces `emit_json()` as the central output function
A new `emit()` function SHALL replace `emit_json()` as the primary output dispatch point. `emit()` SHALL accept a payload and optional `plain_renderer` and `fancy_renderer` callables for curated output.

When a curated renderer is provided and the active style matches, `emit()` SHALL call that renderer instead of the generic fallback.

The legacy `emit_json()` function SHALL remain as a deprecated alias during the transition period.

#### Scenario: emit() with no curated renderer uses generic fallback
- **WHEN** `emit(payload)` is called without curated renderers
- **AND WHEN** the active print style is `plain`
- **THEN** the generic plain fallback renders the payload
