...
When called without `<agent-ref>`, the command SHALL require execution inside the target agent's tmux session, SHALL discover the current tmux session as the attach context, SHALL prefer `AGENTSYS_MANIFEST_PATH` from the current tmux session environment when that pointer is present and valid, SHALL otherwise use `AGENTSYS_AGENT_ID` from that same tmux session to resolve exactly one shared-registry record and manifest path, SHALL require the resolved manifest to belong to the current tmux session, SHALL derive attach authority from that manifest, and SHALL refuse the attach when those inputs are missing, stale, ambiguous, identify a non-`houmao_server_rest` session, or fail to resolve exactly one managed agent on the persisted server authority.

`houmao-mgr` SHALL NOT require the user to address raw `cao_rest` or child-CAO topology in order to attach a gateway for pair-managed sessions, and current-session attach SHALL NOT fall back to gateway-root pointer envs, `terminal_id`, cwd, ambient shell env, or another server target when manifest or shared-registry discovery is invalid or stale.
...
- **THEN** the command resolves exactly one shared-registry record by that authoritative `agent_id`
...
- **AND THEN** it does not retarget the request through gateway-root pointer envs, `terminal_id`, or another alias

#### Scenario: Current-session attach fails closed without usable tmux or registry discovery
...
#### Scenario: Current-session attach fails closed when the resolved manifest authority no longer resolves
- **WHEN** a developer runs `houmao-mgr agents gateway attach` from inside a pair-managed tmux session
- **AND WHEN** the resolved manifest points at persisted attach authority that no longer resolves exactly one managed agent on the persisted server
...
When the resolved manifest declares native headless agent relaunch authority, gateway attach SHALL treat that manifest-owned authority as sufficient to manage future headless turns even when no current headless process pid is published.
...
#### Scenario: Same-session headless gateway surface stays off window 0
- **WHEN** a gateway attach action uses a same-session tmux surface for a native headless session
- **THEN** tmux window `0` remains reserved for headless console output
- **AND THEN** the gateway surface does not claim or replace window `0`
...
Gateway-managed relaunch SHALL always target tmux window `0` for the managed agent surface and SHALL NOT allocate a new tmux window. If a user has repurposed or occupied window `0`, that is outside the contract.
...
### Requirement: `gateway_manifest.json` is outward-facing gateway bookkeeping and is force-refreshed by attach
The system SHALL keep `<session-root>/gateway/gateway_manifest.json` as an outward-facing gateway bookkeeping artifact for external readers, operator tooling, and compatibility surfaces.

Gateway attach SHALL derive `gateway_manifest.json` from manifest-backed authority plus current attach inputs, and SHALL force-overwrite that file on every attach action.

When `gateway_manifest.json` and `manifest.json` disagree, the system SHALL treat `manifest.json` as authoritative for attach and control behavior.

`gateway_manifest.json` SHALL publish the current gateway pid for outside readers, and SHALL NOT be the place where the gateway process stores its own mutable working memory.

#### Scenario: Gateway attach overwrites stale attach publication
- **WHEN** a gateway attach action succeeds for a session whose existing `gateway_manifest.json` contains stale or older bookkeeping data
- **THEN** the attach action rewrites `gateway_manifest.json` from manifest-derived authority for that attach
- **AND THEN** later readers observe the refreshed publication rather than the stale bookkeeping payload

#### Scenario: Attach publication exposes gateway pid rather than agent pid
- **WHEN** a live gateway instance is attached for a session
- **THEN** `gateway_manifest.json` publishes the gateway pid for that live gateway instance
- **AND THEN** the session's agent pid remains part of manifest-owned runtime session truth rather than gateway bookkeeping

#### Scenario: Gateway process keeps working state out of attach publication
- **WHEN** the live gateway handles requests, admission, or recovery work after startup
- **THEN** its evolving operational state is maintained in memory and in the live gateway runtime artifacts
- **AND THEN** `gateway_manifest.json` is not treated as the gateway's mutable internal working store
