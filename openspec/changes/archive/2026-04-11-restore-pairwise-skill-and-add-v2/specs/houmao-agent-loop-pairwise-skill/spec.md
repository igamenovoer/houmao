...
**Reason**: The restored stable pairwise skill no longer owns the enriched lifecycle state vocabulary; that richer observed-state model moves to `houmao-agent-loop-pairwise-v2`.
**Migration**: Use `houmao-agent-loop-pairwise-v2` when canonical observed states such as `initializing`, `awaiting_ack`, `paused`, or `dead` are required.

### Requirement: Pairwise authoring produces standalone participant preparation material
**Reason**: The restored stable pairwise skill no longer owns standalone prestart preparation and per-participant preparation briefs.
**Migration**: Use `houmao-agent-loop-pairwise-v2` when the plan must include explicit prestart material or standalone participant preparation briefs.

### Requirement: Optional downstream timeout-watch policy remains reminder-driven and non-blocking
**Reason**: The restored stable pairwise skill no longer owns the richer reminder-driven timeout-watch guidance.
**Migration**: Use `houmao-agent-loop-pairwise-v2` when timeout-watch policy and overdue follow-up guidance are required.
