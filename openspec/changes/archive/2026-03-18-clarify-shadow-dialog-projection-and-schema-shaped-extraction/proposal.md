...
- Refactor provider shadow parsers so dialog projection is produced by modular, swappable projection processors that operate on already-normalized snapshots, are selected by provider parser-owned version-aware logic, and remain overridable through explicit parser/stack injection points instead of by hardwired monolithic parser methods.
...
- Formalize lifecycle/runtime, operator/diagnostic, and machine-critical reliability tiers so shadow-mode consumers know when projected dialog is acceptable and when schema/sentinel contracts are required.
...
- `shadow-dialog-projection`: revise the projection contract to define best-effort heuristic cleanup, preserve lifecycle/diagnostic usefulness, explicitly deny exact-text guarantees, and codify reliability tiers for repo-owned shadow consumers.
- `versioned-shadow-parser-stack`: refactor the stack so provider/version-specific projection logic is modular and swappable through provider parser-owned projector selection plus explicit stack/parser override pass-through, and route repo-owned shadow-aware helper code through the shared stack-level abstraction.
...
- `cao-claude-demo-scripts`: revise repo-owned Claude/Codex CAO demo-pack success criteria and response-extraction helpers so default shadow-mode runs rely on side effects, explicit shadow-aware extraction, or clearly labeled best-effort text rather than CAO-native reply-text assumptions.
