...
- Remove or update CLI tests, workflow fixtures, and demo-pack CLI consumers that currently expect `agents show` to exist.
...
- `docs-cli-reference`: CLI reference requirements no longer document `agents show` as an active `houmao-mgr` subcommand.
...
- Affected CLI shape tests plus demo or fixture code that shells out to `houmao-mgr agents show`.
