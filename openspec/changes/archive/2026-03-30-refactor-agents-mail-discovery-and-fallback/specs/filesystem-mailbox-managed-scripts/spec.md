...
When the filesystem mailbox transport publishes compatibility Python helper scripts under `rules/scripts/`, it SHALL publish the Python dependency manifest for those helpers under `rules/scripts/requirements.txt`.

When a published compatibility helper uses `pydantic` for request validation, that dependency manifest SHALL declare a `pydantic` requirement using a minimum-version specifier rather than an exact or upper-bounded pin.

The dependency manifest SHALL stay aligned with the imports used by the published compatibility helper set so maintainers can determine the exact Python dependencies required to invoke those helpers safely.

Ordinary agent mailbox workflows SHALL NOT require the agent to inspect that dependency manifest before checking, sending, replying to, or marking mail read.

#### Scenario: Published compatibility helper set includes its dependency manifest
- **WHEN** the runtime publishes compatibility Python mailbox helpers under `rules/scripts/`
- **THEN** the shared mailbox `rules/scripts/requirements.txt` file includes the dependencies required by that helper set
- **AND THEN** the published compatibility assets remain self-describing for maintainers who need to inspect or invoke them

#### Scenario: Dependency manifest does not use an exact or upper-bounded pin for pydantic
- **WHEN** a published compatibility helper set requires `pydantic` in `rules/scripts/requirements.txt`
...
When Python managed mailbox helpers are published under `rules/scripts/`, those helpers SHALL validate request payloads through strict shared schemas before performing SQLite or filesystem mutations.

At minimum, this requirement applies to published compatibility flows for delivery, mailbox-state mutation, repair, mailbox registration, and mailbox deregistration.
...
- **WHEN** a maintainer invokes a published compatibility helper such as `register_mailbox.py` with a payload whose required fields are missing or whose field types are invalid
...
- **WHEN** a maintainer invokes a published compatibility helper such as `deliver_message.py` with a delivery payload containing an invalid nested recipient or attachment field
...
When runtime-managed compatibility helpers are published under `rules/scripts/`, each Python helper SHALL preserve the existing invocation pattern of `--mailbox-root` plus `--payload-file` and SHALL emit exactly one JSON object to stdout for both success and failure outcomes.

Simplifying helper implementation with shared schema-backed parsing SHALL NOT require maintainers who still invoke those compatibility helpers to adopt a different script name, different flags, or an interactive prompt flow.

Repo-owned skills and docs SHALL treat those helpers as compatibility or diagnostic tools rather than as the ordinary mailbox workflow surface.

#### Scenario: Valid compatibility delivery request still uses the existing script contract
- **WHEN** a maintainer invokes a published `deliver_message.py` helper with a valid payload file and mailbox root
...
- **WHEN** a maintainer invokes a published Python managed mailbox helper with an invalid payload file that fails schema validation
...
When Python managed mailbox helpers are published under `rules/scripts/`, those helpers SHALL keep shared mailbox-root catalog state and mailbox-local mailbox-view state consistent during delivery, mailbox-state mutation, and repair.
...
#### Scenario: Published delivery helper initializes sender and recipient local mailbox state
- **WHEN** a maintainer invokes a published `deliver_message.py` helper successfully for one sender and one or more recipients
...
#### Scenario: Published mailbox-state helper updates one addressed mailbox locally
- **WHEN** a maintainer invokes a published `update_mailbox_state.py` helper to mark a message read for one mailbox address
...
#### Scenario: Published repair helper rebuilds local mailbox state when only structural mailbox data survives
...
- **THEN** the published repair flow can recreate deterministic local mailbox state for that mailbox from the available structural mailbox artifacts
