...
#### Scenario: Managed launch gets the default Houmao-owned header section
...
- **AND THEN** that header section identifies the agent as Houmao-managed and names `houmao-mgr` as the canonical direct Houmao interface
...
- **AND THEN** backend-specific prompt injection receives that one structured prompt rather than a separate managed-header bootstrap message
...
- **WHEN** a managed relaunch targets a pre-change manifest that lacks persisted structured prompt-layout metadata
...
#### Scenario: Compatibility-generated profile uses the same structured managed header as local launch
