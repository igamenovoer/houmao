...
### Requirement: Houmao packages a graphing utility system skill
The system SHALL package a Houmao-owned system skill named `houmao-utils-graphing` under the maintained system-skill asset root.
...
#### Scenario: Installed graphing skill explains its scope
- **WHEN** an agent opens the installed `houmao-utils-graphing` skill
...
#### Scenario: System-skill catalog includes the graphing utility
- **WHEN** an operator lists maintained Houmao system skills
- **THEN** `houmao-utils-graphing` appears as an installable Houmao-owned skill
- **AND THEN** it is described as a graphing utility skill rather than as a managed-agent lifecycle skill

### Requirement: Graphing utility provides graphing schema discovery workflows
The `houmao-utils-graphing` skill SHALL instruct agents to discover supported built-in graphing schemas through maintained `houmao-mgr ag-ui impl` surfaces.
...
- **THEN** the graphing utility tells it to run `houmao-mgr ag-ui impl templated-graphics list`
...
- **THEN** the graphing utility tells it to inspect `houmao-mgr ag-ui impl catalog houmao.graphic.template traces`
- **AND THEN** it tells the agent to validate the payload before rendering AG-UI events

### Requirement: Graphing utility teaches Plotly.js templated graphics authoring
The `houmao-utils-graphing` skill SHALL teach agents to prefer `houmao.graphic.template` for supported Plotly.js 2D snapshot charts and chart-like visualizations that fit the Layer 1 trace catalog.
...
- **THEN** the graphing utility directs it to prefer `houmao.graphic.template`
...
- **THEN** the graphing utility tells it not to place that spec in `houmao.graphic.template.extra`
- **AND THEN** it directs the agent to `houmao.graphic.vegalite`

### Requirement: Graphing utility teaches Vega-Lite freeform graphics authoring
The `houmao-utils-graphing` skill SHALL teach agents that `houmao.graphic.vegalite` is the built-in `freeform-graphics` path for custom declarative Vega-Lite graphics.
...
- **THEN** the graphing utility directs it to inspect and use `houmao.graphic.vegalite`
...
- **THEN** the graphing utility tells it to send `chart.to_dict()` or equivalent Vega-Lite JSON
- **AND THEN** it does not tell the agent to send Python code or rely on runtime Python execution by the gateway or workbench

### Requirement: Graphing utility validates and renders graphing payloads before delivery
The `houmao-utils-graphing` skill SHALL instruct agents to validate graphing payloads with `houmao-mgr ag-ui impl validate <implementation> --input <payload>`.
...
- **THEN** the graphing utility tells it to validate the payload through `houmao-mgr ag-ui impl validate houmao.graphic.template`
...
- **THEN** the graphing utility points to the AG-UI interop or gateway workflow for Houmao gateway publishing
- **AND THEN** it does not duplicate gateway routing-id guessing rules as graphing-specific behavior

### Requirement: Graphing utility documents graphing-specific safety limits
The `houmao-utils-graphing` skill SHALL warn agents that graphing payloads must avoid unsafe executable, remote-loading, or credential-bearing content.
...
- **THEN** the graphing utility tells it not to use remote `data.url`
- **AND THEN** it tells the agent to validate the payload before rendering AG-UI events

#### Scenario: Agent keeps unsafe Plotly fields out of Layer 1
- **WHEN** an agent prepares a `houmao.graphic.template` payload
- **THEN** the graphing utility tells it not to use raw Plotly replacement specs or executable content
