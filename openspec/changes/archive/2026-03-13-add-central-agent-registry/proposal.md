...
- Define a versioned secret-free registry record contract for live published agents under `live_agents/`, keyed by the full SHA-256 hex digest of the canonical agent name and carrying a stable per-live-session `generation_id`, soft-lease freshness metadata, runtime and gateway pointers, mailbox identity, and terminal-container hints.
...
- Update runtime-owned session lifecycle and manifest-persisting control flows to publish, refresh, and remove registry records while keeping `manifest.json`, `gateway/attach.json`, and other session-root artifacts authoritative.
- Add stale-cleanup tooling for `live_agents/` directories left behind by unexpected failure, including a minimal operator-facing cleanup entrypoint in v1 while deferring broader registry inspection CLI.
...
- Affected code: runtime lifecycle orchestration, manifest-persisting control flows, session manifest/gateway integration points, mailbox binding publication paths, minimal cleanup CLI plumbing, and new registry storage/model helpers.
