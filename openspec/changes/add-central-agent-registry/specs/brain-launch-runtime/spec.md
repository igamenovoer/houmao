...
### Requirement: Runtime refreshes shared-registry records when gateway or mailbox publication changes
When the runtime materializes or refreshes stable gateway capability for a session, attaches or detaches a live gateway, or refreshes mailbox bindings, it SHALL refresh the corresponding shared-registry record for that same logical session.
