...
Published live-agent directories SHALL live under `~/.houmao/registry/live_agents/<agent-key>/`, where `agent-key` is a deterministic hash of the globally unique published agent name rather than the raw name itself.
...
- a per-process or per-publication `generation_id`,
- publication and lease timestamps,
- identity metadata sufficient to recognize the published agent,
- secret-free pointers to runtime-owned artifacts such as the session manifest path or gateway attach path when available.
...
### Requirement: The system provides cleanup tooling for stale `live_agents/` directories
The system SHALL provide tooling that removes stale directories under `~/.houmao/registry/live_agents/` or the effective override root when those directories no longer correspond to lease-fresh live agents.
