...
**Reason**: Filesystem mailbox root selection still supports explicit configuration and `AGENTSYS_GLOBAL_MAILBOX_DIR`, but mailbox correctness no longer depends on publishing mailbox-specific env bindings such as `AGENTSYS_MAILBOX_FS_ROOT`.
**Migration**: Resolve the effective mailbox root through the persisted runtime mailbox binding or explicit operator inputs, and consume current filesystem mailbox paths through manager-owned resolution rather than mailbox env keys.
