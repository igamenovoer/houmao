...
- Add a filesystem-backed mailbox flavor that stores messages as Markdown files, indexes metadata and mutable status in SQLite, uses filesystem locking instead of a background service, and resolves its mailbox content root from runtime-managed env bindings rather than a fixed run-directory path.
