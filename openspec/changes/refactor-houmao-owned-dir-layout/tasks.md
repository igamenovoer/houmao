...
- [ ] 5.1 Update runtime, registry, launcher, mailbox, and agent-state reference docs to describe the four-zone ownership model, the env-var override surfaces and precedence rules, the new default Houmao runtime root, the per-agent job dir behavior, and the strong-name plus authoritative-`agent_id` association model.
