...
- [ ] 4.6 Configure Dockview for in-app docked movement by keeping drag-and-drop enabled, enabling tab and split movement, and passing `disableFloatingGroups`.
- [ ] 4.7 Omit floating-group and popout-window controls, context-menu actions, and `addFloatingGroup` or `addPopoutGroup` calls.
- [ ] 4.8 Ensure each pane owns independent target configuration, stream controller, event reducer state, and cleanup behavior.
- [ ] 4.9 Persist only layout and non-sensitive target metadata in local storage, sanitize restored Dockview JSON to remove `floatingGroups` and `popoutGroups`, and avoid persisting prompt text or stream payloads by default.
- [ ] 4.10 Add clear empty, connecting, connected, running, error, and disconnected states without in-app tutorial copy.
...
- [ ] 6.3 Add a Playwright smoke path that creates at least two agent panes, moves a pane into an in-app split, connects them to separate fake targets, and verifies event isolation between panes.
- [ ] 6.4 Add a Playwright assertion that no floating group or popout window is created during pane add, pane move, layout save, or layout restore.
- [ ] 6.5 Add a Playwright assertion for visible `houmao_render_graphic` SVG title, alt text, and SVG content.
- [ ] 6.6 Add a Playwright assertion that closing or disconnecting a pane uses GUI detach or abort behavior and does not expect Houmao interrupt semantics.
- [ ] 6.7 Add a Playwright assertion that layout and target metadata restore after reload while prior stream payloads do not persist.
...
- [ ] 7.2 Document that live/manual validation for this change should use a Kimi Code headless Houmao agent, preferably with `tests/fixtures/auth-bundles/kimi/personal-a-default/` when credentials are present.
- [ ] 7.3 Document that the GUI does not manage Houmao agent lifecycle and that close/stop means GUI detach by default.
- [ ] 7.4 Update `context/plans/ag-ui-integration/roadmap.md` with the workbench milestone, the Kimi Code headless live-validation lane, and what remains after it.
- [ ] 7.5 Run the workbench TypeScript typecheck and production build.
- [ ] 7.6 Run the deterministic Bun Playwright workbench E2E smoke.
- [ ] 7.7 Run or document the opt-in Kimi Code headless live-agent workbench smoke when local credentials are available.
- [ ] 7.8 Run focused Python AG-UI tests if backend contract adjustments are needed during implementation.
- [ ] 7.9 Run `openspec validate add-ag-ui-workbench-app --strict`.
