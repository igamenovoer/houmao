...
- [ ] 1.1 Add `houmao-server` HTTP models for the full supported `cao-server` route surface, additive compatibility-safe request/response extensions, Houmao-owned new endpoints, and child-CAO metadata
...
- [ ] 1.3 Add a local `houmao-server` entrypoint implemented with Python `click` plus a client layer for health, session, terminal, and Houmao extension operations
...
- [ ] 4.1 Add `houmao-srv-ctrl` as a Python `click`-based full supported-`cao`-compatible command family that delegates most commands to the installed `cao` executable while allowing only additive optional Houmao extensions
...
- [ ] 6.1 Add parity coverage against a real `cao-server` for the full supported HTTP API, including request handling, status codes, CAO-defined response fields, additive-extension safety, child-CAO lifecycle, watch-worker lifecycle, and runtime routing
- [ ] 6.2 Add parity coverage against a real `cao` CLI for the full supported command family through `houmao-srv-ctrl`, including exit status, user-visible output shape, additive-extension safety, and alias-style drop-in usage
...
- [ ] 6.4 Add reference docs describing `houmao-server`, `houmao-srv-ctrl`, the child-CAO shallow-cut architecture, the hidden Houmao-owned child-storage model, the filesystem-vs-memory persistence boundary, the transitional shared-registry bridge, and the migration path toward eventual CAO replacement
