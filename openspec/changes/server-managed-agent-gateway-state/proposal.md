...
- Extend the native headless launch contract so mailbox and gateway behavior can be configured as part of the official server API rather than only through prebuilt manifests.
...
- `houmao-server-agent-api`: Expand the managed-agent API with transport-neutral request submission, richer headless launch inputs, detailed state inspection, and server-owned gateway control surfaces.
...
- Expected follow-on:
  - a dual-agent async mailbox ping-pong demo under `scripts/demo/` can be implemented on top of this official contract without introducing demo-specific server behavior
