...
**Reason**: Replaced by the managed-agent persist lane under the unified workspace envelope.
...
**Reason**: Replaced by workspace root, scratch lane, persist binding, and persist lane inspection.
**Migration**: None. Backward compatibility and migration are explicitly out of scope for this change.

### Requirement: Houmao does not define or clean memory-directory contents
**Reason**: Replaced by lane-specific workspace ownership and cleanup boundaries.
**Migration**: None. Backward compatibility and migration are explicitly out of scope for this change.
