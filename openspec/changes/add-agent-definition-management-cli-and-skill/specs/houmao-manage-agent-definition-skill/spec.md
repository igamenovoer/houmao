...
That skill SHALL instruct agents to manage low-level project-local agent definitions through these supported commands:
...
- `houmao-mgr project agents roles presets set`
...
- `project agents tools <tool> auth list|get|add|set|remove` when the user is asking to mutate auth-bundle contents rather than a preset's auth reference
- direct hand-editing under `.houmao/agents/roles/`

#### Scenario: Installed skill points the agent at low-level role and preset commands
- **WHEN** an agent opens the installed `houmao-manage-agent-definition` skill
- **THEN** the skill directs the agent to use the supported `project agents roles ...` and `project agents roles presets ...` command surfaces for low-level definition work
...
### Requirement: `houmao-manage-agent-definition` selects the correct role or preset command and asks before guessing
...
- use `roles scaffold` for creating one structurally complete starter slice across one or more tool lanes
- use `roles presets add` for adding one preset lane to an existing role
- use `roles list` for listing roles
- use `roles presets list` for listing preset lanes for one role
...
- use `roles presets get` for inspecting one preset lane
- use `roles set` for changing the canonical role prompt
- use `roles presets set` for changing preset auth reference, skill membership, or prompt mode
- use `roles remove` for removing one role
- use `roles presets remove` for removing one preset lane

At minimum, the skill SHALL require the agent to obtain:

- for `roles list`: no role name
- for `roles presets list`: the role name
- for `roles get`: the role name
- for `roles presets get`: the role name and tool, plus setup when not `default`
- for `roles init`: the role name, and optional initial preset inputs only when the user asked for them explicitly
- for `roles scaffold`: the role name and at least one tool lane
- for `roles set`: the role name and at least one explicit prompt mutation
- for `roles presets add`: the role name and tool, plus any optional preset fields the user explicitly wants to author
- for `roles presets set`: the role name, tool, and at least one explicit preset mutation
- for removal: the concrete role or preset target to delete

When the user asks to change which credential bundle one definition uses, the skill SHALL treat that as a preset-auth-reference change on `roles presets set` rather than as direct auth-bundle mutation.
...
- **THEN** the skill directs the agent to use `houmao-mgr project agents roles presets set --auth ...` or `--clear-auth`
...
- **AND THEN** it does not guess a role, preset lane, tool, or field change
