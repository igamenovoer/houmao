...
### Requirement: Active `Houmao` CAO guidance references the fork repository
Active `Houmao` operational guidance, knowledge-base notes, and implementation-facing references SHALL use the CAO fork repository as the canonical source reference.
...
- **WHEN** a maintainer updates active `Houmao` CAO docs, specs, or issue notes
...
### Requirement: Active `Houmao` install guidance uses a fork-backed source
Active `Houmao` installation, prerequisite, troubleshooting, and `uvx --from` guidance SHALL use a fork-backed CAO source.
...
- **WHEN** a user follows active CAO install or `uvx --from` guidance from `Houmao`
...
`Houmao` SHALL allow archive, provenance, or licensing text to remain explicit about original upstream CAO origin when that text is intentionally documenting history or attribution.
...
- **THEN** active operational guidance in `Houmao` no longer depends on orphan/upstream CAO source references
