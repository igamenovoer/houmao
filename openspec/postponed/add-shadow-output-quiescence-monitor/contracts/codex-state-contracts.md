...
This note defines the Codex parser-state contract for the `add-shadow-output-quiescence-monitor` change.

It focuses on:

- how Codex snapshot states are defined,
- which transition facts the Codex parser may emit across ordered snapshots,
- how those facts relate to runtime-owned quiescence handling, and
- which claims Codex parsing must not make.
...
> Codex parsing owns one-snapshot interpretation. Runtime owns time-based quiescence and submit-aware lifecycle.

Codex parsing therefore owns:
...
|-------|------|--------------|
| `CodexShadowParser` | one-snapshot Codex state/projection | quiet-window timing |
| Runtime quiescence layer | tmux change detection and quiet timers | provider regexes and Codex syntax |
...
- Codex parser can say `ready_for_input`.
...
- `V`: resolved Codex parser preset version
...
Codex binds the shared state contract with these values:
...
- no blocking Codex context such as selection/approval UI applies.

For this change, `accepts_input=true` means “Codex looks safe,” not “runtime must submit immediately.”

## Version-Bound Detection Predicates

Each Codex preset version `V` supplies concrete detectors for these placeholders.

| Placeholder | Meaning |
|-------------|---------|
| `SUPPORTED_OUTPUT_FAMILY(V)` | Snapshot matches a supported Codex output family |
| `CODEX_IDLE_PROMPT(V)` | Snapshot shows a Codex prompt that is input-ready |
| `CODEX_PROCESSING_SIGNAL(V)` | Snapshot shows Codex actively processing |
| `CODEX_SELECTION_MENU(V)` | Snapshot shows a user-choice menu |
| `CODEX_APPROVAL_PROMPT(V)` | Snapshot shows approval/trust UI |
| `CODEX_SLASH_COMMAND_CONTEXT(V)` | Snapshot shows active slash-command UI |
| `CODEX_ERROR_BANNER(V)` | Snapshot shows Codex-side error UI |
| `DISCONNECTED_SIGNAL(V)` | Snapshot shows detachment/loss of terminal |
| `DIALOG_LINE_KEEP(V)` | Line stays in projected dialog |
| `DIALOG_LINE_DROP(V)` | Line is Codex chrome and should be dropped |
...
Codex parser transitions are defined across ordered snapshots.
...
This change adds a strict runtime boundary around Codex parser transitions:

- `evt_prompt_ready` means the Codex surface now looks ready.
...
Runtime derives these additional facts from ordered Codex observations:

| Runtime Fact | Meaning |
|--------------|---------|
| `evt_transport_changed` | normalized Codex snapshot changed |
...
```mermaid
flowchart TD
    A[Codex snapshot n] --> B[Codex parser]
...
Codex dialog projection should:

- preserve supported label-style and TUI-style visible dialog in order,
- remove Codex prompt/footer/spinner chrome,
- provide `head` and `tail` slices,
- record projection provenance and anomalies.

Codex dialog projection should not:
...
The Codex parser contract must not claim:
