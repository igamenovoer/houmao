# claude-code-state-tracking-interactive-watch Specification

## Purpose
Define the interactive Claude state-tracking watch workflow, including fixture-backed launch, live dashboard behavior, retained artifacts, and final offline analysis.
...
The repository SHALL provide an interactive Claude state-tracking watch workflow under `scripts/explore/claude-code-state-tracking/` that can build a fresh Claude runtime home from canonical preset-backed fixture inputs under `tests/fixtures/agents/`, launch the generated `launch.sh` in tmux, start recorder-backed observation, and bring up a live dashboard for the same run.
...
- a Claude launch path driven by a tracked role preset together with the matching tool setup bundle and selected auth bundle from the same agent-definition root,
...
- **THEN** the workflow builds a fresh Claude runtime home from canonical preset-backed repository fixtures and launches its generated `launch.sh` in tmux together with recorder-backed observation and a live dashboard
