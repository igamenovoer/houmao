...
The system SHALL provide a local-only Codex auth bundle named `yunwu-openai` that stores the launch-time API key and endpoint routing values in `env/vars.env` using the env var names accepted by the current Codex adapter.

#### Scenario: Auth env file carries launch-time values
- **WHEN** a developer prepares `tests/fixtures/agents/tools/codex/auth/yunwu-openai/env/vars.env`
