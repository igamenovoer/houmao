...
For a selected discovered agent with `has_gateway = true`, the workbench SHALL request `GET /houmao/agents/{agent_ref}/gateway` from the configured passive server before applying the selection.

When the gateway status includes `gateway_host` and `gateway_port`, the workbench SHALL derive an AG-UI target URL in the form `http://{gateway_host}:{gateway_port}/v1/ag-ui`, with wildcard gateway hosts normalized to a browser-reachable host derived from the passive-server URL.

When the selected agent has no gateway, has unresolved gateway coordinates, or returns an agent-not-found, ambiguous-agent, no-gateway, unreachable-gateway, or proxy-policy error, the workbench SHALL leave the current pane target unchanged and show the resolution error.

#### Scenario: Selected agent resolves to AG-UI URL
- **WHEN** a discovered agent row has `has_gateway = true`
- **AND WHEN** the passive-server gateway endpoint returns `gateway_host = "127.0.0.1"` and `gateway_port = 9012`
- **THEN** the workbench derives `http://127.0.0.1:9012/v1/ag-ui` as the AG-UI target URL for that selection

#### Scenario: Unresolved gateway does not retarget pane
- **WHEN** a tester selects an agent whose gateway status returns 502 because no gateway is attached
- **THEN** the workbench shows the gateway resolution error
- **AND THEN** the currently configured pane target URL is unchanged
