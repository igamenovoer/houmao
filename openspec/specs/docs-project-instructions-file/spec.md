# docs-project-instructions-file Specification

## Purpose

Define accuracy requirements for the repo-root `CLAUDE.md` project-instructions file so that it reflects the current Houmao build/run phase vocabulary, the current `src/houmao/` subsystem layout, and the current operator entrypoints, and so that it never presents retired CAO-era workflows as current.
