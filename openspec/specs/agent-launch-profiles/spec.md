...
Each stored memo seed SHALL have one policy:
- `initialize`, which is the default,
- `replace`,
- `fail-if-nonempty`.

Launch-profile inspection SHALL report whether a memo seed is present, the stored policy, and the seed source kind without printing full memo or page contents by default.
