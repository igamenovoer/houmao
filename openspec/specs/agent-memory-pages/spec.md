...
For policy `replace`, a represented pages target SHALL clear then rewrite only the contained `pages/` tree. An empty `pages/` directory in the seed SHALL represent an intentional request to clear pages.

#### Scenario: Pages-only replace leaves memo unchanged
