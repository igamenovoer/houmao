...
## Purpose
Define the requirements for repairing demo and tutorial launch paths so they start successfully against the current agent-definition model.
...
Demo-owned config, state, or report fields MAY temporarily keep names such as `recipe_path`, `brain_recipe_path`, or `blueprint` during this change. Those compatibility-only fields SHALL no longer force startup to depend on legacy recipe or blueprint source trees.
...
- **AND THEN** that compatibility field SHALL NOT require restoring legacy recipe or blueprint launch semantics
