...
For this change, the `stalwart` transport SHALL support the same shared mailbox operation set exposed through the gateway mailbox facade and used by direct mailbox workflows:
...
The `stalwart` transport SHALL NOT require the shared gateway or direct mailbox operation contract to expose transport-specific folder, archive, delete, move, or star semantics in this change.

#### Scenario: Shared mailbox operation set stays limited to common operations
- **WHEN** a caller uses shared mailbox interaction against a `stalwart` mailbox-enabled session
- **THEN** the supported operation set is limited to mailbox status, `check`, `send`, and `reply`
- **AND THEN** transport-specific mailbox features outside that common set are not required by this change
