...
## Purpose
Define the higher-level `houmao-mgr project` workflow for compiling reusable specialist definitions into the canonical repo-local `.houmao/agents/` tree.
## Requirements
...
### Requirement: `project specialist create` compiles one specialist into canonical project agent artifacts
`houmao-mgr project specialist create` SHALL create one project-local specialist by persisting the operator's intended specialist semantics into the active project-local catalog and managed content store.
...
- **AND WHEN** an operator runs `houmao-mgr project specialist create --name researcher --tool codex --api-key sk-test`
...
`houmao-mgr project specialist get --name <specialist>` SHALL report one specialist's high-level semantic metadata plus the managed content or derived artifact references relevant to that specialist.
...
`houmao-mgr project specialist remove --name <specialist>` SHALL remove the persisted specialist definition from the project-local catalog and SHALL remove any specialist-owned derived projection state that exists only for that specialist.
...
`houmao-mgr project specialist get` SHALL report persistent specialist env records separately from the credential selection and auth content path.

Maintained `houmao-mgr project specialist list`, `get`, and `remove` flows SHALL resolve the active overlay through the shared non-creating project-aware resolver.
...
`project specialist remove` SHALL remain non-creating even though it mutates existing specialist state.
...
- **AND WHEN** an operator runs `houmao-mgr project specialist get --name researcher`
...
- **AND WHEN** an operator runs `houmao-mgr project specialist remove --name researcher`
...
- **WHEN** no active project overlay exists
- **AND WHEN** an operator runs `houmao-mgr project specialist list`
...
### Requirement: `project profile create/list/get/remove` manages specialist-backed project profiles
`houmao-mgr project profile create --name <profile> --specialist <specialist>` SHALL persist one reusable project profile that targets exactly one existing specialist.

Project profiles SHALL be specialist-backed birth-time launch configuration owned by the easy lane.

`project profile create` SHALL accept `--gateway-mail-notifier-appendix-text <text>` to store a reusable notifier appendix default on that project profile.

When no active project overlay exists for the caller and no stronger overlay selection override is supplied, `project profile create` SHALL ensure `<cwd>/.houmao` exists before persisting profile state.

`project profile list`, `get`, and `remove` SHALL resolve the active overlay through the shared non-creating project-aware resolver and SHALL fail clearly when no active overlay exists.

`project profile get --name <profile>` SHALL report the source specialist plus the stored project-profile launch defaults, including the stored notifier appendix default when present.

`project profile remove --name <profile>` SHALL remove only the profile definition and SHALL NOT remove the referenced specialist only because that specialist was the profile source.

#### Scenario: Project profile create bootstraps the missing overlay on demand
- **WHEN** no active project overlay exists
- **AND WHEN** an operator runs `houmao-mgr project profile create --name alice --specialist cuda-coder`
...
#### Scenario: Project profile create stores notifier appendix default
- **WHEN** an operator runs `houmao-mgr project profile create --name alice --specialist cuda-coder --gateway-mail-notifier-appendix-text "Watch billing-related inbox items first."`
- **THEN** the stored project profile records that notifier appendix default
- **AND THEN** later `project profile get --name alice` reports the stored appendix default

#### Scenario: Project profile remove preserves the referenced specialist
- **WHEN** project profile `alice` targets specialist `cuda-coder`
- **AND WHEN** an operator runs `houmao-mgr project profile remove --name alice`
...
### Requirement: `project` surfaces support unified model configuration
`houmao-mgr project specialist create` SHALL accept optional `--model <name>` as a launch-owned default model selection for the created specialist.

`houmao-mgr project specialist create` SHALL accept optional `--reasoning-level <integer>=non-negative` as a launch-owned default reasoning preset index for the created specialist.
...
`houmao-mgr project profile create` SHALL accept optional `--model <name>` as a reusable project-profile model override.

`houmao-mgr project profile create` SHALL accept optional `--reasoning-level <integer>=non-negative` as a reusable project-profile reasoning override.

`houmao-mgr project agents launch` SHALL accept optional `--model <name>` and `--reasoning-level <integer>=non-negative` as one-off launch overrides for either `--specialist` or `--profile` launch.

For project-profile-backed launch, the effective model configuration SHALL resolve with this precedence:

1. stored specialist or source recipe default
2. project-profile model override
3. direct `project agents launch` override

Direct easy-instance override SHALL NOT rewrite the stored specialist or project profile.

#### Scenario: Specialist create persists launch-owned model configuration
- **WHEN** an operator runs `houmao-mgr project specialist create --name reviewer --tool codex --api-key sk-test --model gpt-5.4 --reasoning-level 3`
...
#### Scenario: Project profile create stores a reusable model override
- **WHEN** specialist `reviewer` already exists with source model `gpt-5.4`
- **AND WHEN** an operator runs `houmao-mgr project profile create --name reviewer-fast --specialist reviewer --model gpt-5.4-mini --reasoning-level 2`
- **THEN** the stored project profile records model override `gpt-5.4-mini` and reasoning override `2`
- **AND THEN** those values are treated as project-profile launch configuration rather than as credential state

#### Scenario: Direct easy-instance model override wins over the project-profile default
- **WHEN** project profile `reviewer-fast` stores model override `gpt-5.4-mini`
- **AND WHEN** an operator runs `houmao-mgr project agents launch --profile reviewer-fast --name reviewer-fast-1 --model gpt-5.4-nano`
- **THEN** the resulting launch uses model `gpt-5.4-nano`
- **AND THEN** the stored project profile still records `gpt-5.4-mini` as its reusable default

#### Scenario: Direct easy-instance reasoning override wins over the project-profile default
- **WHEN** project profile `reviewer-fast` stores reasoning override `2`
- **AND WHEN** an operator runs `houmao-mgr project agents launch --profile reviewer-fast --name reviewer-fast-1 --reasoning-level 12`
...
- **AND WHEN** an operator runs `houmao-mgr project agents launch --specialist researcher --name repo-research-1`
...
- **AND WHEN** an operator runs `houmao-mgr project agents launch --specialist researcher --name repo-research-1 --workdir /repo-b`
...
### Requirement: `project agents launch` supports project-profile-backed launch
`houmao-mgr project agents launch` SHALL support selecting a reusable project profile through `--profile <profile>`.

`--profile` and `--specialist` SHALL be mutually exclusive on this surface.

When `--profile` is used, the command SHALL derive the source specialist from the stored profile, SHALL apply project-profile defaults before direct CLI overrides, and SHALL still use the selected project overlay as the authoritative source context.
...
- **AND WHEN** an operator runs `houmao-mgr project agents launch --profile alice`
...
#### Scenario: Direct CLI overrides still win for project-profile-backed launch
- **WHEN** project profile `alice` stores auth override `alice-creds` and workdir `/repos/alice-cuda`
- **AND WHEN** an operator runs `houmao-mgr project agents launch --profile alice --auth breakglass --workdir /tmp/debug`
...
### Requirement: `project agents launch` defaults gateway attach with explicit per-launch overrides
`houmao-mgr project agents launch` SHALL request launch-time gateway attach by default unless the operator explicitly passes `--no-gateway`.
...
If launch-time gateway attach fails after the managed session has already started, `project agents launch` SHALL keep the session running and SHALL report the attach failure explicitly together with the launched session identity needed for retry.

#### Scenario: Default easy launch attaches a loopback foreground gateway with a system-assigned port
- **WHEN** an operator runs `houmao-mgr project agents launch --specialist researcher --name repo-research-1`
...
- **WHEN** an operator runs `houmao-mgr project agents launch --specialist researcher --name repo-research-1 --no-gateway`
...
- **WHEN** an operator runs `houmao-mgr project agents launch --specialist researcher --name repo-research-1 --gateway-port 43123`
...
- **WHEN** an operator runs `houmao-mgr project agents launch --specialist researcher --name repo-research-1 --gateway-background`
- **THEN** the command requests launch-time gateway attach for that launch
...
- **WHEN** an operator runs `houmao-mgr project agents launch --specialist researcher --name repo-research-1 --no-gateway --gateway-background`
...
- **WHEN** `houmao-mgr project agents launch` starts the managed session successfully
...
### Requirement: `project agents list/get/stop` presents runtime state by specialist and wraps existing runtime stop control

`houmao-mgr project agents list` SHALL present launched managed agents as instances, annotated by their originating specialist when that specialist can be resolved.

`houmao-mgr project agents get --name <instance>` SHALL report the current managed-agent runtime summary plus the originating specialist metadata when available.

`houmao-mgr project agents stop --name <instance>` SHALL stop one managed-agent instance after verifying that the resolved runtime session belongs to the current project overlay.

`project agents stop` SHALL delegate to the existing canonical managed-agent stop implementation rather than directly killing the resolved tmux session.

This change SHALL NOT define `project agents stop` semantics that differ from the current managed-agent stop behavior.
...
Maintained `houmao-mgr project agents list`, `get`, and `stop` flows SHALL resolve the active overlay through the shared non-creating project-aware resolver before inspecting runtime state or verifying overlay ownership.

When no active project overlay exists for the caller and no stronger overlay selection override applies, these commands SHALL fail clearly without bootstrapping a new overlay.

When the resolved runtime state includes a mailbox association, `project agents get` SHALL report the effective mailbox summary, including:
...
`project agents list` SHALL surface whether each instance currently has a mailbox association and MAY present that information as a compact mailbox summary.
...
- **AND WHEN** an operator runs `houmao-mgr project agents list`
...
- **AND WHEN** an operator runs `houmao-mgr project agents get --name repo-research-1`
...
- **AND WHEN** an operator runs `houmao-mgr project agents stop --name repo-research-1`
...
### Requirement: Easy instance inspection reports project-profile origin when available
When a managed-agent instance was launched from a project-project profile, `project agents list` and `project agents get` SHALL report that originating project-profile identity when it is resolvable from runtime-backed state.

The instance inspection surface SHALL continue to report the originating specialist when available.

#### Scenario: Easy instance get reports both project-profile origin and specialist origin
- **WHEN** instance `alice` was launched from project profile `alice`
- **AND WHEN** that profile targets specialist `cuda-coder`
- **AND WHEN** an operator runs `houmao-mgr project agents get --name alice`
- **THEN** the command reports project profile `alice` as the originating reusable birth-time configuration
- **AND THEN** it also reports specialist `cuda-coder` as the underlying reusable source

### Requirement: `project specialist create` persists explicit tool setup selection
`houmao-mgr project specialist create` SHALL accept an optional `--setup <name>` input with a default of `default`.
...
This requirement SHALL apply to any project-specialist tool that uses setup bundles today or in the future.

#### Scenario: Explicit non-default setup is persisted through specialist creation
- **WHEN** an operator runs `houmao-mgr project specialist create --name researcher --tool codex --setup yunwu-openai --api-key sk-test`
...
- **WHEN** an operator runs `houmao-mgr project specialist create --name reviewer --tool claude --api-key sk-test`
...
Maintained `houmao-mgr project ...` help text, failures, and ownership-mismatch errors SHALL use the same selected-overlay and non-creating terminology as the broader project-aware contract.
...
- **WHEN** no active project overlay exists
- **AND WHEN** an operator runs `houmao-mgr project specialist list` or `houmao-mgr project agents list`
...
- **WHEN** an operator runs a maintained `houmao-mgr project agents get` or `stop` command
...
### Requirement: Claude specialists support vendor OAuth token and imported login state
`houmao-mgr project specialist create --tool claude` SHALL accept Claude vendor-auth inputs for:

- `CLAUDE_CODE_OAUTH_TOKEN`
- imported Claude login state from a Claude config root

When provided, the command SHALL persist those inputs into the selected or derived Claude credential bundle using the same Claude auth-bundle contract as `houmao-mgr internals native-agent tools claude auth`.
...
#### Scenario: Specialist create persists the Claude OAuth-token lane
- **WHEN** an operator runs `houmao-mgr project specialist create --name reviewer --tool claude --claude-oauth-token token123`
...
#### Scenario: Specialist create imports Claude login state from a config root
- **WHEN** `/tmp/claude-home` contains vendor Claude login-state files
- **AND WHEN** an operator runs `houmao-mgr project specialist create --name reviewer --tool claude --claude-config-dir /tmp/claude-home`
...
#### Scenario: Specialist create preserves other explicit Claude settings with imported login state
- **WHEN** an operator runs `houmao-mgr project specialist create --name reviewer --tool claude --claude-config-dir /tmp/claude-home --claude-model claude-opus`
...
### Requirement: `project profile create` supports stored managed-header policy
`houmao-mgr project profile create` SHALL accept:
...
When neither flag is supplied, the created project profile SHALL store managed-header policy `inherit`.

`project profile get --name <profile>` SHALL report the stored managed-header policy.

#### Scenario: Project profile create stores disabled managed-header policy
- **WHEN** an operator runs `houmao-mgr project profile create --name reviewer-fast --specialist reviewer --no-managed-header`
- **THEN** the created project profile stores managed-header policy `disabled`
- **AND THEN** later `project profile get --name reviewer-fast` reports that stored policy

### Requirement: `project agents launch` supports one-shot managed-header override
`houmao-mgr project agents launch` SHALL accept:
...
When neither flag is supplied, easy-instance launch SHALL inherit managed-header policy from the selected project profile when one is present, otherwise from the system default.

Direct one-shot managed-header override SHALL NOT rewrite the stored project profile.

#### Scenario: Easy-instance launch disables the managed header for one profile-backed launch
- **WHEN** project profile `reviewer-fast` stores managed-header policy `enabled`
- **AND WHEN** an operator runs `houmao-mgr project agents launch --profile reviewer-fast --no-managed-header`
- **THEN** the resulting managed launch does not prepend the managed prompt header
- **AND THEN** project profile `reviewer-fast` still records managed-header policy `enabled`

#### Scenario: Easy-instance launch enables the managed header despite stored disabled policy
- **WHEN** project profile `reviewer-fast` stores managed-header policy `disabled`
- **AND WHEN** an operator runs `houmao-mgr project agents launch --profile reviewer-fast --managed-header`
- **THEN** the resulting managed launch prepends the managed prompt header
- **AND THEN** project profile `reviewer-fast` still records managed-header policy `disabled`

### Requirement: `project agents launch` supports launch-owned managed force takeover

`houmao-mgr project agents launch` SHALL accept optional `--force` for delegated managed takeover on the current easy-launch invocation.
...
The selected force mode SHALL be forwarded to the delegated native managed launch for the current invocation only and SHALL NOT be persisted into the stored specialist or project profile.
...
When `--force` is supplied, easy instance launch SHALL request the corresponding managed runtime takeover for the resolved managed identity whether that identity comes from direct `--name` or from an project-profile default.

#### Scenario: Bare `--force` defaults to `keep-stale` for specialist-backed launch
- **WHEN** an operator runs `houmao-mgr project agents launch --specialist researcher --name repo-research-1 --force`
...
- **WHEN** project profile `alice` stores default managed-agent name `alice`
- **AND WHEN** an operator runs `houmao-mgr project agents launch --profile alice --force clean`
...
- **AND THEN** stored project profile `alice` remains unchanged and does not gain a persisted force mode
...
### Requirement: `project agents launch` supports one-shot launch-owned system-prompt appendix
`houmao-mgr project agents launch` SHALL accept optional launch-owned system-prompt appendix input through:
...
When either option is supplied, the provided appendix SHALL affect only the current easy-instance launch and SHALL NOT rewrite the stored specialist or project profile.

When the selected project profile already contributes a launch-profile prompt overlay, the appendix SHALL be appended after overlay resolution within the delegated native managed launch.

#### Scenario: Easy-profile-backed launch appends one-shot prompt text without rewriting the profile
- **WHEN** project profile `alice` stores a reusable prompt overlay
- **AND WHEN** an operator runs `houmao-mgr project agents launch --profile alice --append-system-prompt-text "Treat gateway diagnostics as high priority."`
- **THEN** the current easy-instance launch appends that prompt text after the resolved profile overlay
- **AND THEN** project profile `alice` remains unchanged after the launch

#### Scenario: Specialist-backed launch appends file-based prompt content for one launch
- **WHEN** an operator runs `houmao-mgr project agents launch --specialist researcher --name repo-research-1 --append-system-prompt-file /tmp/appendix.md`
...
- **WHEN** an operator supplies both `--append-system-prompt-text` and `--append-system-prompt-file` on one `houmao-mgr project agents launch` invocation
...
### Requirement: `project` auth relationships resolve through auth profile identity
`houmao-mgr project` SHALL resolve specialist-selected auth and project-profile auth overrides through auth profile identity rather than through auth display-name text or auth projection path names as the authoritative key.

`project specialist create --credential <name>` SHALL continue to accept a display name for auth selection or creation.

When `--credential` is omitted, the existing `<specialist-name>-creds` behavior SHALL remain as a display-name default only.

Easy-specialist inspection and project-profile-backed launch SHALL render or accept current auth display names while preserving the underlying auth-profile relationship across auth rename.
...
- **AND WHEN** an operator runs `houmao-mgr project specialist get --name reviewer`
...
- **WHEN** project profile `alice` stores an auth override referencing one auth profile currently named `alice-creds`
...
### Requirement: `project profile set` patches specialist-backed project profiles
`houmao-mgr project profile set --name <profile>` SHALL patch one existing specialist-backed project profile in the active project overlay.

The command SHALL preserve the profile source specialist and SHALL preserve unspecified stored launch defaults.

At minimum, `project profile set` SHALL support the same stored launch-default field families as `internals native-agent launch-dossiers set`, including managed-agent identity defaults, workdir, auth, memory binding, model, reasoning level, prompt mode, env records, mailbox config, launch posture, managed-header policy, prompt overlay, and gateway mail-notifier appendix default.
...
#### Scenario: Project profile set updates auth without dropping prompt overlay
- **WHEN** project profile `alice` targets specialist `reviewer` and stores auth override `work` plus prompt overlay text
- **AND WHEN** an operator runs `houmao-mgr project profile set --name alice --auth breakglass`
- **THEN** project profile `alice` stores auth override `breakglass`
- **AND THEN** project profile `alice` still stores its prior prompt overlay text

#### Scenario: Project profile set clears prompt overlay
- **WHEN** project profile `alice` stores prompt overlay mode `append` with prompt overlay text
- **AND WHEN** an operator runs `houmao-mgr project profile set --name alice --clear-prompt-overlay`
- **THEN** project profile `alice` no longer stores prompt overlay mode or prompt overlay text
...
#### Scenario: Project profile set clears notifier appendix default
- **WHEN** project profile `alice` stores gateway mail-notifier appendix default
- **AND WHEN** an operator runs `houmao-mgr project profile set --name alice --clear-gateway-mail-notifier-appendix`
- **THEN** project profile `alice` no longer stores a notifier appendix default
- **AND THEN** future launches from `alice` do not inherit a profile-owned notifier appendix unless another source supplies one

#### Scenario: Project profile set rejects empty update
- **WHEN** project profile `alice` exists
- **AND WHEN** an operator runs `houmao-mgr project profile set --name alice` without any update or clear flags
- **THEN** the command fails clearly
- **AND THEN** project profile `alice` remains unchanged

### Requirement: `project profile create --yes` replaces same-lane profiles
`houmao-mgr project profile create --name <profile> --specialist <specialist> --yes` SHALL replace an existing same-name project profile in the active project overlay.
...
When the same-name project profile already exists and replacement confirmation is not supplied, the command SHALL prompt on interactive terminals or fail in non-interactive contexts with guidance to rerun using `--yes`.

When the same-name profile exists but is not an project profile, the command SHALL fail clearly even when `--yes` is supplied.

#### Scenario: Project profile create requires confirmation before replacement
- **WHEN** project profile `alice` already exists
- **AND WHEN** an operator runs `houmao-mgr project profile create --name alice --specialist reviewer` in a non-interactive context without `--yes`
- **THEN** the command fails clearly with guidance to rerun using `--yes`
- **AND THEN** project profile `alice` remains unchanged

#### Scenario: Project profile create yes replaces and clears omitted fields
- **WHEN** project profile `alice` targets specialist `reviewer` and stores workdir `/repos/alice` plus prompt overlay text
- **AND WHEN** an operator runs `houmao-mgr project profile create --name alice --specialist reviewer-v2 --workdir /repos/alice-v2 --yes`
- **THEN** project profile `alice` targets specialist `reviewer-v2` and stores workdir `/repos/alice-v2`
- **AND THEN** project profile `alice` no longer stores the prior prompt overlay text

#### Scenario: Project profile create yes rejects cross-lane conflict
- **WHEN** explicit launch profile `alice` already exists
- **AND WHEN** an operator runs `houmao-mgr project profile create --name alice --specialist reviewer --yes`
- **THEN** the command fails clearly because `alice` is not an project profile
...
After `project profile set` or same-lane `project profile create --yes` replacement updates stored profile state, the command SHALL rematerialize the project agent catalog projection.

The projected `.houmao/agents/launch-profiles/<profile>.yaml` resource SHALL reflect the updated stored profile.

#### Scenario: Project profile set updates projected launch profile
- **WHEN** project profile `alice` projects to `.houmao/agents/launch-profiles/alice.yaml`
- **AND WHEN** an operator runs `houmao-mgr project profile set --name alice --workdir /repos/alice-next`
- **THEN** the stored project profile records workdir `/repos/alice-next`
- **AND THEN** the projected `.houmao/agents/launch-profiles/alice.yaml` reflects workdir `/repos/alice-next`

### Requirement: Project profiles store managed-header section policy
`houmao-mgr project profile create` and `houmao-mgr project profile set` SHALL accept repeatable managed-header section policy options using `--managed-header-section SECTION=STATE`.
...
`houmao-mgr project profile set` SHALL also accept:
...
#### Scenario: Project profile create stores disabled automation notice
- **WHEN** an operator runs `houmao-mgr project profile create --name reviewer-fast --specialist reviewer --managed-header-section automation-notice=disabled`
- **THEN** project profile `reviewer-fast` stores automation notice section policy `disabled`
...
#### Scenario: Project profile set clears one section policy
- **WHEN** project profile `reviewer-fast` stores automation notice section policy `disabled` and identity section policy `disabled`
- **AND WHEN** an operator runs `houmao-mgr project profile set --name reviewer-fast --clear-managed-header-section identity`
- **THEN** project profile `reviewer-fast` no longer stores an identity section policy
- **AND THEN** project profile `reviewer-fast` still stores automation notice section policy `disabled`

#### Scenario: Project profile get reports stored section policy
- **WHEN** project profile `reviewer-fast` stores automation notice section policy `disabled`
- **AND WHEN** an operator runs `houmao-mgr project profile get --name reviewer-fast`
...
#### Scenario: Project profile create enables default-off mail acknowledgement
- **WHEN** an operator runs `houmao-mgr project profile create --name mailer-fast --specialist reviewer --managed-header-section mail-ack=enabled`
- **THEN** project profile `mailer-fast` stores mail acknowledgement section policy `enabled`
...
`houmao-mgr project agents launch` SHALL accept repeatable one-shot managed-header section overrides using `--managed-header-section SECTION=STATE`.
...
When neither `--managed-header-section` nor whole-header `--managed-header` / `--no-managed-header` is supplied, easy-instance launch SHALL inherit managed-header section policy from the selected project profile when one is present, otherwise from the section default.

Direct one-shot managed-header section overrides SHALL influence only the current launch and SHALL NOT rewrite stored project-profile state.
...
- **WHEN** an operator runs `houmao-mgr project agents launch --profile reviewer-fast --managed-header-section automation-notice=disabled`
...
#### Scenario: Easy-instance section override does not rewrite project profile
- **WHEN** project profile `reviewer-fast` stores automation notice section policy `disabled`
- **AND WHEN** an operator runs `houmao-mgr project agents launch --profile reviewer-fast --managed-header-section automation-notice=enabled`
- **THEN** the resulting managed launch includes the automation notice section
- **AND THEN** project profile `reviewer-fast` still records automation notice section policy `disabled`

#### Scenario: Easy-instance launch enables mail acknowledgement for one launch
- **WHEN** an operator runs `houmao-mgr project agents launch --profile reviewer-fast --managed-header-section mail-ack=enabled`
- **THEN** the resulting managed launch keeps the managed header enabled
- **AND THEN** the resulting managed launch includes the mail acknowledgement section
- **AND THEN** project profile `reviewer-fast` is not rewritten
...
- **AND WHEN** an operator runs `houmao-mgr project agents launch --specialist reviewer`
- **AND WHEN** the whole managed header is not disabled
- **THEN** the resulting managed launch includes the automation notice section
- **AND THEN** `as_is` does not disable agent-facing automation guidance

### Requirement: `project specialist set` patches existing specialists
`houmao-mgr project specialist set --name <specialist>` SHALL patch one existing project-local specialist in the active project overlay.

The command SHALL preserve unspecified stored specialist fields by default.

At minimum, `project specialist set` SHALL support mutation of specialist prompt content, skill bindings, setup selection, credential display-name selection, launch prompt mode, launch-owned model name, launch-owned reasoning level, and persistent specialist env records.
...
- **AND WHEN** an operator runs `houmao-mgr project specialist set --name researcher --system-prompt "You are a focused repo researcher."`
...
- **AND WHEN** an operator runs `houmao-mgr project specialist set --name researcher` without any update or clear flags
- **THEN** the command fails clearly
...
- **THEN** `project specialist set` does not provide a `--tool` mutation path
...
`houmao-mgr project specialist set --name <specialist>` SHALL allow operators to edit specialist skill bindings without respecifying the entire specialist.
...
`project specialist set --with-skill` SHALL NOT delete, move, rewrite, or partially consume the provided source directory, even when the underlying project skill registration refreshes an existing canonical entry or fails partway through.
...
- **AND WHEN** an operator runs `houmao-mgr project specialist set --name researcher --with-skill /tmp/notes-skill`
...
- **AND WHEN** an operator runs `houmao-mgr project specialist set --name researcher --with-skill /repo/skillset/notes-skill`
- **THEN** specialist `researcher` stores skill binding `notes-skill`
...
- **AND WHEN** an operator runs `houmao-mgr project specialist set --name researcher --remove-skill notes`
...
`houmao-mgr project specialist set --name <specialist>` SHALL allow operators to update launch-owned source defaults stored on the specialist.
...
- **AND WHEN** an operator runs `houmao-mgr project specialist set --name reviewer --model gpt-5.4-mini`
...
- **AND WHEN** an operator runs `houmao-mgr project specialist set --name researcher --env-set OPENAI_API_KEY=sk-test`
...
After `project specialist set` updates stored specialist state, the command SHALL rematerialize the project agent catalog projection.
...
`project specialist set` SHALL NOT mutate running managed-agent sessions, runtime homes, or already-written launch manifests in place.
...
- **AND WHEN** an operator runs `houmao-mgr project specialist set --name researcher --add-skill notes`
...
- **AND WHEN** an operator runs `houmao-mgr project specialist set --name researcher --system-prompt "Use the new review policy."`
...
### Requirement: `project agents launch` exposes gateway TUI tracking timings for auto-attach
`houmao-mgr project agents launch` SHALL accept optional one-shot gateway TUI tracking timing overrides for launch-time gateway auto-attach.
...
Supplying gateway TUI timing overrides SHALL NOT rewrite the stored specialist or project-profile launch defaults.

#### Scenario: Easy launch passes timing overrides to gateway auto-attach
- **WHEN** an operator runs `houmao-mgr project agents launch --specialist researcher --name repo-research-1 --gateway-tui-completion-stability-seconds 2.5`
...
- **WHEN** an operator runs `houmao-mgr project agents launch --specialist researcher --name repo-research-1 --gateway-tui-final-stable-active-recovery-seconds 30`
- **AND WHEN** launch-time gateway auto-attach is enabled
...
- **WHEN** an operator launches from project profile `alice` with one or more `--gateway-tui-*` timing overrides
- **THEN** the launch uses those timing overrides for that launch-time gateway attach
- **AND THEN** the stored project profile remains unchanged

#### Scenario: Easy launch rejects timing overrides when gateway attach is disabled
- **WHEN** an operator runs `houmao-mgr project agents launch --specialist researcher --name repo-research-1 --no-gateway --gateway-tui-stale-active-recovery-seconds 10`
...
### Requirement: `project` surfaces use memo-pages managed memory
`houmao-mgr project profile create` and `project agents launch` SHALL NOT accept `--persist-dir` or `--no-persist-dir`.

`project agents get` SHALL report memory root, memo file, and pages directory when available.

`project profile get` SHALL NOT report reusable persist-lane defaults.

#### Scenario: Project profile create rejects persist-dir
- **WHEN** an operator runs `houmao-mgr project profile create --name alice --specialist cuda-coder --persist-dir ../shared/alice-persist`
...
- **WHEN** an operator runs `houmao-mgr project agents get --name researcher-1`
...
### Requirement: Project profile CLI manages memo seeds
`houmao-mgr project profile create` SHALL accept at most one memo seed source option:
- `--memo-seed-text <text>`,
- `--memo-seed-file <path>`,
- `--memo-seed-dir <path>`.

`houmao-mgr project profile create` SHALL NOT accept a memo seed apply policy option.

`houmao-mgr project profile set` SHALL support the same source options. It SHALL also accept `--clear-memo-seed` to remove the stored memo seed from the project profile.

`--clear-memo-seed` SHALL NOT be combined with a memo seed source.

`project profile get --name <profile>` SHALL report memo seed presence, source kind, and managed content reference metadata without printing full memo or page contents by default.

`project agents launch --profile <profile>` SHALL apply the selected project profile's memo seed during managed launch when the profile stores one, using source-scoped replacement semantics from managed launch runtime.

#### Scenario: Project profile create stores memo seed text
- **WHEN** an operator runs `houmao-mgr project profile create --name reviewer-default --specialist reviewer --memo-seed-text "Read pages/review.md first."`
- **THEN** the project profile stores the supplied text as a memo seed
...
#### Scenario: Project profile set preserves memo seed when patching workdir
- **WHEN** project profile `reviewer-default` stores a memo seed
- **AND WHEN** an operator runs `houmao-mgr project profile set --name reviewer-default --workdir /repos/next`
- **THEN** the profile updates the stored workdir
- **AND THEN** the stored memo seed remains unchanged

#### Scenario: Project profile set replaces memo seed content
- **WHEN** project profile `reviewer-default` stores a memo seed
- **AND WHEN** an operator runs `houmao-mgr project profile set --name reviewer-default --memo-seed-file docs/reviewer-next.md`
...
- **WHEN** project profile `reviewer-default` stores memo-only seed text
- **AND WHEN** managed agent `reviewer-default` already has pages
- **AND WHEN** an operator runs `houmao-mgr project agents launch --profile reviewer-default`
...
- **THEN** `project profile create` or `project profile set` fails before mutating the stored profile

#### Scenario: Project profile set rejects clear combined with source
- **WHEN** an operator runs `houmao-mgr project profile set --name reviewer-default --clear-memo-seed --memo-seed-file docs/reviewer.md`
- **THEN** the command fails clearly before mutating the project profile

### Requirement: Easy instance stop preserves managed-agent cleanup locators
`houmao-mgr project agents stop --name <name>` SHALL preserve durable cleanup locator fields from the underlying managed-agent stop result when they are available.
...
- **WHEN** an operator runs `houmao-mgr project agents stop --name reviewer`
...
Maintained `houmao-mgr project ...` specialist flows SHALL NOT silently import or rewrite legacy `.houmao/easy/specialists/*.toml` metadata during ordinary specialist inspection, mutation, or launch preparation.
...
### Requirement: `project specialist create` can bind registered project skills
`houmao-mgr project specialist create` SHALL support binding existing registered project skills by name at create time.
...
`project specialist create --with-skill` SHALL NOT delete, move, rewrite, or partially consume the provided source directory, even when the underlying project skill registration refreshes an existing canonical entry or the create command later fails.
...
- **AND WHEN** an operator runs `houmao-mgr project specialist create --name researcher --tool codex --api-key sk-test --skill notes`
...
- **AND WHEN** an operator runs `houmao-mgr project specialist create --name researcher --tool codex --api-key sk-test --with-skill /repo/skillset/notes`
- **THEN** specialist `researcher` stores a binding to registered project skill `notes`
...
`houmao-mgr project specialist get --name <specialist>` SHALL report each bound project skill by registered name.
...
### Requirement: `project profile` enforces project-profile lane ownership
`houmao-mgr project profile list|get|set|remove` SHALL operate only on stored `easy_profile` entries even though native launch dossiers share the same catalog-backed launch-profile family and compatibility projection path.

When `project profile get --name <profile>`, `set --name <profile>`, or `remove --name <profile>` targets a stored profile whose `profile_lane` is `launch_profile`, the command SHALL fail clearly instead of reading, mutating, or deleting that explicit launch profile through the easy lane.

That wrong-lane failure SHALL identify that the named resource belongs to the explicit launch-profile lane and SHALL direct the operator to the corresponding `houmao-mgr internals native-agent launch-dossiers <verb> --name <profile>` command.

`project profile list` SHALL continue returning only project-profile entries in `profiles`. When that easy-lane result is empty and one or more native launch dossiers exist in the selected overlay, the output SHALL include operator guidance to use `houmao-mgr internals native-agent launch-dossiers list`.
...
- **AND WHEN** an operator runs `houmao-mgr project profile get --name nightly`
...
- **AND THEN** the error directs the operator to `houmao-mgr internals native-agent launch-dossiers get --name nightly`

#### Scenario: Easy set rejects explicit launch profile with redirect guidance
- **WHEN** explicit launch profile `nightly` exists in the selected project overlay
- **AND WHEN** an operator runs `houmao-mgr project profile set --name nightly --workdir /repos/nightly-next`
- **THEN** the command fails clearly before mutating `nightly`
- **AND THEN** the error explains that `nightly` belongs to the explicit launch-profile lane
- **AND THEN** the error directs the operator to `houmao-mgr internals native-agent launch-dossiers set --name nightly`

#### Scenario: Easy remove rejects explicit launch profile with redirect guidance
- **WHEN** explicit launch profile `nightly` exists in the selected project overlay
- **AND WHEN** an operator runs `houmao-mgr project profile remove --name nightly`
- **THEN** the command fails clearly before deleting `nightly`
- **AND THEN** the error explains that `nightly` belongs to the explicit launch-profile lane
- **AND THEN** the error directs the operator to `houmao-mgr internals native-agent launch-dossiers remove --name nightly`

#### Scenario: Easy list keeps lane filtering and adds note when only native launch dossiers exist
- **WHEN** the selected project overlay contains one or more native launch dossiers
- **AND WHEN** the selected project overlay contains no project profiles that match the current easy-list query
- **AND WHEN** an operator runs `houmao-mgr project profile list`
- **THEN** the structured output reports an empty `profiles` list
- **AND THEN** the structured output includes guidance to use `houmao-mgr internals native-agent launch-dossiers list`
- **AND THEN** the easy list output does not inline native launch dossiers under `profiles`

### Requirement: `project agents launch` supports explicit preserved-home reuse

`houmao-mgr project agents launch` SHALL accept optional `--reuse-home` for the current easy launch.

`--reuse-home` SHALL be launch-owned only and SHALL NOT be persisted into the stored specialist or project profile.
...
The command SHALL support `--reuse-home` for both specialist-backed launch and project-profile-backed launch.

For project-profile-backed launch, reused-home restart SHALL apply the currently stored project-profile inputs, together with any stronger direct CLI overrides, onto the preserved home before startup. Updating the stored project profile after the prior run SHALL therefore affect the restarted agent even though the prior stopped instance was not mutated in place.
...
- **WHEN** project profile `reviewer-default` resolves managed identity `reviewer-default`
...
- **AND WHEN** the stored project profile `reviewer-default` has been updated since the prior run
- **AND WHEN** an operator runs `houmao-mgr project agents launch --profile reviewer-default --reuse-home`
- **THEN** the delegated native launch requests reused-home restart for that managed identity
- **AND THEN** the current stored project-profile inputs are projected onto the preserved home before startup
...
- **WHEN** an operator runs `houmao-mgr project agents launch --specialist reviewer --name reviewer-a --reuse-home`
...
- **AND WHEN** an operator runs `houmao-mgr project agents launch --specialist reviewer --name reviewer-a --reuse-home`
...
- **WHEN** project profile `reviewer-default` resolves one stopped compatible preserved home
- **AND WHEN** the stopped lifecycle metadata carries prior tmux session name `HOUMAO-reviewer-default-1700000000000`
- **AND WHEN** an operator runs `houmao-mgr project agents launch --profile reviewer-default --reuse-home --session-name reviewer-restart-debug`
...
### Requirement: Project profiles manage registered and private skill overlays
`houmao-mgr project profile create` and `houmao-mgr project profile set` SHALL support storing profile-owned skill overlays without mutating the referenced source specialist.

The project-profile surface SHALL accept repeatable `--add-registered-skill <name>` options. Each registered skill name SHALL reference an existing project skill registration by name and SHALL NOT create, import, copy, or symlink a new project skill registration.

The project-profile surface SHALL accept repeatable `--remove-registered-skill <name>` options on `set`. Removing a registered skill SHALL remove only that project-profile-owned reference and SHALL NOT remove the project skill registration.

The project-profile surface SHALL accept repeatable `--add-private-skill <path>` options. Each private skill path SHALL identify a directory containing `SKILL.md`, SHALL derive its installed skill name from that directory name, and SHALL be stored with copy mode.

The project-profile surface SHALL accept repeatable `--add-private-skill-symlink <path>` options. Each private skill path SHALL identify a directory containing `SKILL.md`, SHALL derive its installed skill name from that directory name, and SHALL be stored with symlink mode.

The project-profile surface SHALL accept repeatable `--remove-private-skill <path>` options on `set`. Removing a private skill SHALL remove only the matching project-profile-owned private skill reference and SHALL NOT mutate the referenced source directory.
...
Adding the same private installed skill name more than once in a single project profile SHALL fail clearly before mutating profile state.

#### Scenario: Project profile create stores registered and private skill overlays
- **WHEN** project skill `llm-wiki` is registered
- **AND WHEN** `/repo/profile-skills/audit/SKILL.md` exists
- **AND WHEN** an operator runs `houmao-mgr project profile create --name reviewer-a --specialist reviewer --add-registered-skill llm-wiki --add-private-skill /repo/profile-skills/audit`
- **THEN** project profile `reviewer-a` stores registered skill ref `llm-wiki`
...
#### Scenario: Project profile set patches private symlink skill
- **WHEN** project profile `reviewer-a` exists
- **AND WHEN** `/repo/profile-skills/live-tools/SKILL.md` exists
- **AND WHEN** an operator runs `houmao-mgr project profile set --name reviewer-a --add-private-skill-symlink /repo/profile-skills/live-tools`
- **THEN** project profile `reviewer-a` stores private skill `live-tools` with mode `symlink`
- **AND THEN** project skill `live-tools` is not added to the project skill registry

#### Scenario: Project profile rejects unknown registered skill
- **WHEN** project skill `unknown-skill` is not registered
- **AND WHEN** an operator runs `houmao-mgr project profile create --name reviewer-a --specialist reviewer --add-registered-skill unknown-skill`
...
### Requirement: Project profile inspection and projection report skill overlays
`houmao-mgr project profile get --name <profile>` SHALL report stored registered skill refs and private skill refs as part of the profile defaults.

`houmao-mgr project profile list` SHALL include enough structured skill-overlay summary data for operators to see that a profile contributes additional skills.

The projected `.houmao/agents/launch-profiles/<profile>.yaml` file for an project profile SHALL render registered skill refs separately from private skill refs. Private skill refs SHALL include installed name, source path, and mode.

#### Scenario: Project profile get reports registered and private skill overlays
- **WHEN** project profile `reviewer-a` stores registered skill `llm-wiki`
- **AND WHEN** it stores private skill `audit` from `/repo/profile-skills/audit` with mode `copy`
- **AND WHEN** an operator runs `houmao-mgr project profile get --name reviewer-a`
...
#### Scenario: Project profile projection renders skill overlays
- **WHEN** project profile `reviewer-a` stores registered skill `llm-wiki`
...
### Requirement: Project profile env records are regression-tested through Codex TUI launch
The test suite SHALL include coverage for a profile-backed Codex TUI launch where durable env records are created through the public project-profile CLI surface and then observed inside the launched tmux-backed runtime.

The test SHALL create an project profile with repeatable `project profile create --env-set NAME=value` inputs, launch a managed agent from that profile with `project agents launch --profile <name>` using the Codex local interactive lane, and verify the profile env records reach the launched tmux session or provider process environment.

The test SHALL use non-secret env names and values. It SHALL include lowercase proxy env names matching the issue report.

#### Scenario: Profile env reaches Codex TUI launched from project profile
- **WHEN** a project contains a Codex-backed specialist and an project profile created with `--env-set http_proxy=http://127.0.0.1:7990`, `--env-set https_proxy=http://127.0.0.1:7990`, and `--env-set FEATURE_FLAG_X=profile-env`
- **AND WHEN** an operator launches the profile through `houmao-mgr project agents launch --profile <profile>`
...
- **THEN** it creates the profile through `project profile create --env-set` rather than direct catalog mutation
- **AND THEN** it launches through `project agents launch --profile` rather than calling the runtime backend directly
