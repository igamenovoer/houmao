...
1. **Require explicit selection.** Accept `<area>/<profile>`, a bare area aliasing `/normal`, `all/<profile>`, `tag:<name>`, exact case or variant ids, or a comma-separated composite union. With no selector, return the read-only suite summary and stop.
2. **Resolve and preview the suite.** Expand cumulative profiles and variants in catalog order, deduplicate overlaps, record every selection source and exclusion, and show case, variant, provider, repetition, and total-attempt counts. Reject unknown selectors before launch.
...
7. **Return selection, qualification posture, outcome counts, flaky cases, stable failures, inconclusive cases, activation-unobserved cases, evidence root, and cleanup status.**
