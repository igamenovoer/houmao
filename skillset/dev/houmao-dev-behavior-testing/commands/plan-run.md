...
1. **Resolve suite selectors.** Read [../references/case-catalog.md](../references/case-catalog.md) and [../references/case-schema.md](../references/case-schema.md); accept area/profile, global profile, tag, exact case, exact variant, or composite union forms. With no selector, return read-only selection help and stop without creating a run root.
2. **Expand deterministically.** Load only selected functional-area pages, expand cumulative tiers and stable variants in catalog order, union and deduplicate `(case_id, variant_id)` cells, and retain every contributing selector. Reject unknown selectors before launch.
...
6. **Declare cost and boundaries.** Preview resolved case and variant counts, provider/context cells, repetitions, total attempts, timeouts, evidence sources, allowed mutation roots, and cleanup obligations.
7. **Write and freeze `run-manifest.json`.** Include ordered requested selectors, catalog version and digest, resolved cases and variants, functional-area/profile attribution, contributing selectors, explicit exclusions, provider/context/repetition matrices, Git/source posture, and content digests.
