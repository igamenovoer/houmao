...
def native_agent_brain_group() -> None:
...
@native_agent_brain_group.command(name="build")
@click.option(
    "--native-agent-root",
    type=click.Path(path_type=Path, file_okay=False, dir_okay=True),
    default=None,
    help=f"Native-agent root to build from. Defaults to `{NATIVE_AGENT_ROOT_ENV_VAR}`.",
)
@click.option("--tool", default=None, help="Tool identifier used by the selected adapter.")
@click.option("--skill", "skills", multiple=True, help="Skill path or name to project.")
@click.option("--setup", default=None, help="Setup bundle to materialize.")
...
@click.option(
    "--preset",
    default=None,
    help="Preset path or bare preset name resolved from the native-agent root.",
)
@click.option(
    "--runtime-root",
    default=None,
...
def build_native_agent_brain_command(
    ...
    """Build one local brain home from internal native-agent material."""

    cwd = Path.cwd().resolve()
    try:
        native_resolution = resolve_native_agent_root(
        ...
        raise click.ClickException(str(exc)) from exc
    resolved_agent_def_dir = native_resolution.root
    preset_path: Path | None = None
    preset_payload = None
    if preset is not None:
        preset_path = resolve_explicit_or_named_preset_path(
    ...
    emit(
        {
            ...
            "manifest_path": str(result.manifest_path),
            "runtime_root": str(resolved_runtime_root),
            "runtime_root_detail": _native_runtime_root_detail(runtime_root),
            "native_agent_root": str(resolved_agent_def_dir),
            "native_agent_root_source": native_resolution.source,
            "native_agent_root_diagnostics": list(native_resolution.diagnostics),
        }
    )
...
def _native_runtime_root_detail(explicit_root: str | None) -> str:
    """Describe runtime-root selection for direct native-agent build plumbing."""

    if explicit_root is not None:
        return "Selected runtime root from the explicit `--runtime-root` override."
    return "Selected shared runtime root for internal native-agent brain build."
