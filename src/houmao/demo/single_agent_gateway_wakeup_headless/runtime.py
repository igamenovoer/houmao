"""Runtime and `houmao-mgr` helpers for the headless gateway wake-up demo."""
...
def launch_instance(
    *,
    ...
    """Launch one headless managed-agent instance from the selected specialist."""
...
def capture_gateway_console(
    *,
    ...
    fallback_window_index: str | None,
    timeout_seconds: float,
    lines: int,
) -> dict[str, Any]:
    ...
    window_index = status_payload.get("gateway_tmux_window_index") or fallback_window_index
...
def expose_project_mailbox_skills(
    *,
    ...
    The runtime-owned mailbox skills are installed into each tool's native home.
    This demo intentionally avoids staging a second project-local `skills/`
    mirror so the live contract stays on the native skill surface.
    """
