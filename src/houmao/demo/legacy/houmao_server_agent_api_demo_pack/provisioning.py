"""Provisioning primitives for the Houmao-server agent API demo pack.

This module owns the direct `houmao-server` live API mechanics used by the
demo pack and its HTT harness. It starts one isolated `houmao-server`,
launches real Claude and Codex managed-agent lanes across TUI and headless
transports, verifies the public HTTP routes, and preserves artifacts under one
demo-owned run root.
"""
...
DEFAULT_DEMO_PACK_RELATIVE_DIR = Path("scripts/demo/houmao-server-agent-api-demo-pack")


class SuiteError(RuntimeError):
    """Raised when the demo-pack live workflow cannot complete successfully."""
...
@dataclass(frozen=True)
class FixturePaths:
    """Tracked demo-pack asset paths consumed by the live workflow."""

    repo_root: Path
    pack_dir: Path
    agent_def_dir: Path
    compatibility_profile_path: Path | None
    dummy_project_fixture: Path


@dataclass(frozen=True)
class SuitePaths:
    """Resolved demo-owned output paths for one live run."""
...
class LaneRuntime:
    ...
    interrupt_verification: dict[str, Any] | None = None
    ...
    def summary_payload(self) -> dict[str, Any]:
        ...
        return {
            ...
            "interrupt_verification": self.interrupt_verification,
...
@dataclass(frozen=True)
class SuiteConfig:
    """Operator-provided configuration for one demo-pack live run."""
...
def run_suite(config: SuiteConfig) -> dict[str, Any]:
    """Run the full demo-pack live workflow and return the final summary payload."""

    _validate_config(config)
    selected_lanes = _resolve_selected_lanes(config.selected_lane_ids)
    fixtures = _resolve_fixture_paths(config.pack_dir)
    ...
    _write_json(paths.control_dir / "config.json", _config_payload(config=config, paths=paths))
    ...
    try:
        ...
        _write_json(paths.control_dir / "summary.json", final_summary)
...
def _resolve_fixture_paths(pack_dir: Path | None = None) -> FixturePaths:
    """Resolve the tracked demo-pack asset paths used by the live workflow."""

    repo_root = Path(__file__).resolve().parents[4]
    resolved_pack_dir = (
        pack_dir.resolve()
        if pack_dir is not None
        else (repo_root / DEFAULT_DEMO_PACK_RELATIVE_DIR).resolve()
    )
    return FixturePaths(
        repo_root=repo_root,
        ...
        dummy_project_fixture=(resolved_pack_dir / "inputs" / "project-template").resolve(),
    )


def _build_suite_paths(*, pack_dir: Path, output_root: Path | None, run_slug: str) -> SuitePaths:
...
def load_suite_paths(run_root: Path) -> SuitePaths:
    ...
    return SuitePaths(
        ...
        runtime_root=(resolved_run_root / "runtime").resolve(),
        registry_root=(resolved_run_root / "registry").resolve(),
        jobs_root=(resolved_run_root / "jobs").resolve(),
        home_dir=(resolved_run_root / "home").resolve(),
        logs_dir=(resolved_run_root / "logs").resolve(),
        server_logs_dir=(resolved_run_root / "logs" / "server").resolve(),
        server_runtime_root=(resolved_run_root / "server-runtime").resolve(),
        suite_http_dir=(resolved_run_root / "control" / "http").resolve(),
        server_dir=(resolved_run_root / "control" / "server").resolve(),
        lanes_root=(resolved_run_root / "lanes").resolve(),
    )
...
def _start_suite_server(
    *,
    ...
    """Start the demo-owned houmao-server subprocess and wait for health."""
...
def _verify_shared_routes(
    *,
    ...
    return {
        "listed_agent_ids": sorted(returned_ids),
        "expected_agent_ids": sorted(tracked_agent_ids),
        "missing_agent_ids": missing,
        "history_limit": history_limit,
        "list_response": _json_ready(list_response),
    }


def _verify_lane_routes(
    *,
    client: HoumaoServerClient,
    lane_runtime: LaneRuntime,
    history_limit: int,
    ...
    history = _record_route_call(
        recorder=lane_runtime.http_recorder,
        label="get-managed-agent-history",
        method="GET",
        path=f"/houmao/agents/{tracked_agent_id}/history",
        request_payload={"limit": history_limit},
        callback=lambda: client.get_managed_agent_history(
            tracked_agent_id,
            limit=history_limit,
        ),
    )
...
def _submit_prompt_and_verify(
    *,
    ...
    prompt = (
        ...
        else _DEFAULT_PROMPT_TEMPLATE.format(
...
def _submit_interrupt_and_verify(
    *,
    client: HoumaoServerClient,
    config: SuiteConfig,
    lane_runtime: LaneRuntime,
) -> None:
    """Submit one interrupt through `/requests` and verify follow-up state evidence."""

    tracked_agent_id = _require_tracked_agent_id(lane_runtime)
    state_before = _record_route_call(
        recorder=lane_runtime.http_recorder,
        label="state-before-interrupt",
        method="GET",
        path=f"/houmao/agents/{tracked_agent_id}/state",
        request_payload=None,
        callback=lambda: client.get_managed_agent_state(tracked_agent_id),
    )
    history_before = _record_route_call(
        recorder=lane_runtime.http_recorder,
        label="history-before-interrupt",
        method="GET",
        path=f"/houmao/agents/{tracked_agent_id}/history",
        request_payload={"limit": config.history_limit},
        callback=lambda: client.get_managed_agent_history(
            tracked_agent_id,
    ...
    history_after = _record_route_call(
        recorder=lane_runtime.http_recorder,
        label="history-after-interrupt",
        method="GET",
        path=f"/houmao/agents/{tracked_agent_id}/history",
        request_payload={"limit": config.history_limit},
        callback=lambda: client.get_managed_agent_history(
    ...
    lane_runtime.interrupt_verification = {
        "accepted": _json_ready(accepted),
        "state_before": _json_ready(state_before),
        "history_before": _json_ready(history_before),
        "state_after": _json_ready(state_after),
        "history_after": _json_ready(history_after),
    }
    _write_json(
        lane_runtime.lane_root / "interrupt-verification.json",
        lane_runtime.interrupt_verification,
    )
