"""Kimi headless backend (`kimi -p` + prompt-mode resume selectors)."""
...
class KimiHeadlessSession(HeadlessInteractiveSession):
    """Kimi Code resumable prompt-mode headless backend."""
    ...
    def _build_command(
        ...
        *,
        ...
        if not _args_include_option(command, "--skills-dir"):
            command.extend(["--skills-dir", str(self._plan.home_path / "skills")])
        command.extend(["-p", effective_prompt])
        if not _args_include_option(command, "--output-format"):
...
def _args_include_option(args: list[str], option: str) -> bool:
    """Return whether split or equals-form args already include one option."""
