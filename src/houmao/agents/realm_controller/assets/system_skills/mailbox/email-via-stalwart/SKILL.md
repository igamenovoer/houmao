...
description: Operate the Stalwart-backed mailbox transport through manager-owned live mailbox discovery, the shared gateway mailbox facade when available, and `houmao-mgr agents mail ...` fallback when it is not.
license: MIT
---

# Email Via Stalwart

Use this skill when the resolved mailbox transport is `stalwart`.
