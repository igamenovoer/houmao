...
5. Resolve the durable start-charter page path under `HOUMAO_AGENT_PAGES_DIR` through `houmao-memory-mgr`, using a run-scoped namespace such as `loop-runs/pairwise-v3/<run_id>/start-charter.md`.
...
12. After acceptance, initialize or refresh the runtime-owned recovery record under `<runtime-root>/loop-runs/pairwise-v2/<run_id>/record.json` and append a `start_accepted` event to `events.jsonl`:
   ...
   - record `run_id`, `recovery_epoch=0`, canonical plan reference, plan revision or digest, designated master, allowed participant set, durable initialize/start page references, mailbox bindings, declared workspace contract summary, declarative wakeup posture, and recoverable-versus-terminal state
...
- page namespace: `loop-runs/pairwise-v3/<run_id>/start-charter.md`
...
Accepted pairwise-v3 starts create or refresh one runtime-owned recovery record for that logical run:
...
The recovery record should capture the accepted plan reference and freshness marker, participant set, durable page references, mailbox bindings, declared workspace contract summary, declarative wakeup posture, and whether the run is still recoverable.
...
- The authored workspace contract travels with the accepted run, but runtime-owned recovery files remain outside that contract.
