...
6. For each rebound participant whose managed memory is being used, resolve one run-scoped continuation page path under `HOUMAO_AGENT_PAGES_DIR` through `houmao-memory-mgr`, using a namespace such as `loop-runs/pairwise-v3/<run_id>/recover-and-continue.md`.
