...
| Mailbox follow-up | `agents mail resolve-live`, then prefer the live gateway mailbox skill when `gateway.base_url` exists, otherwise `agents mail status|check|send|reply|mark-read` | `GET /mail/resolve-live`, then prefer direct `{gateway.base_url}/v1/mail/*` when a live gateway mailbox facade exists, otherwise `/houmao/agents/{agent_ref}/mail/*` | Prefer live gateway-backed mailbox work for shared outgoing mail when available. |
