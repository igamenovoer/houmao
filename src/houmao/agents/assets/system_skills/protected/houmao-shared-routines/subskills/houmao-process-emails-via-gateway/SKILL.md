...
- `$<public-entrypoint> process-emails-via-gateway help`
- `$<public-entrypoint> process-emails-via-gateway process the notifier round at <gateway_base_url>`
- `$<public-entrypoint> process-emails-via-gateway process unread_only open mail`
- `$<public-entrypoint> process-emails-via-gateway process this mail round with reply hardening`

Related skills and boundaries:

- Use `<public-entrypoint>->houmao-shared-routines->agent-email-comms` for endpoint discovery, exact `/v1/mail/*` contracts, transport-local fallback, or ordinary mailbox actions.
- Use `<public-entrypoint>->houmao-shared-routines->agent-gateway` for gateway lifecycle, notifier configuration, or reminder management outside this round.
- Use `<public-entrypoint>->houmao-shared-routines->adv-usage-pattern` for designing notifier-driven loop posture beyond one round.
...
- If you need the exact gateway route contract for `status`, `list`, `peek`, `read`, `send`, `reply`, `mark`, `move`, or `archive`, use the installed Houmao skill `<public-entrypoint>->houmao-shared-routines->agent-email-comms`.
- Use the transport-local guidance inside `<public-entrypoint>->houmao-shared-routines->agent-email-comms` only for transport-specific context or no-gateway fallback.
