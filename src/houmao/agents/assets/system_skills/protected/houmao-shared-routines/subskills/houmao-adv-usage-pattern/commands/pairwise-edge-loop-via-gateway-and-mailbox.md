...
Use `<public-entrypoint>->houmao-shared-routines->agent-loop-lite` instead when the user explicitly wants to package this kind of small loop as generated skills with typed Markdown templates and direct SQLite state.

Use `<public-entrypoint>->houmao-shared-routines->agent-loop-pro` in `tree-loop` mode instead when the user needs recursive child loops, multiple local-close edges, a rendered control graph, schema-rich generated execplans, or generated loop run-control actions.
...
- `<public-entrypoint>->houmao-shared-routines->agent-messaging` for queued gateway prompt handoff between already-running managed agents
- `<public-entrypoint>->houmao-shared-routines->agent-email-comms` for receipt email, final result email, and final-result acknowledgement email
- `<public-entrypoint>->houmao-shared-routines->agent-gateway` for live supervisor reminders
- `<public-entrypoint>->houmao-shared-routines->agent-inspect` for read-only downstream worker peeking before due resends
...
This page intentionally covers only the elemental two-node round. Use `<public-entrypoint>->houmao-shared-routines->agent-loop-lite` for a small generated-skill package with typed Markdown templates and direct SQLite state, or use `<public-entrypoint>->houmao-shared-routines->agent-loop-pro` in `tree-loop` mode for recursive child edge-loops, multiple local-close edges, schema-rich generated execplans, or a rendered control graph.
...
2. Send the queued request prompt to the worker through `<public-entrypoint>->houmao-shared-routines->agent-messaging`.
...
3. if the expected signal is still missing and the loop is due, use `<public-entrypoint>->houmao-shared-routines->agent-inspect` to peek the downstream worker for the same `edge_loop_id`,
...
5. if read-only inspection is unavailable, stale, or inconclusive and the resend decision remains ambiguous, use a narrow active prompt, ping, or direct `<public-entrypoint>->houmao-shared-routines->agent-messaging` status probe only as a last resort before resend,
...
If the worker needs a recursive tree loop, multiple downstream workers, or a rendered control graph, use `<public-entrypoint>->houmao-shared-routines->agent-loop-pro` instead of expanding this elemental protocol inline. If the operator wants only a small reusable generated-skill package around the elemental protocol, use `<public-entrypoint>->houmao-shared-routines->agent-loop-lite`.
...
Prompt: Reopen the active edge-loop ledger, check mailbox first for edge-loop receipts, results, and result acknowledgements, advance completed rows, use `<public-entrypoint>->houmao-shared-routines->agent-inspect` to peek workers for due rows, use a narrow active status probe only as the last resort when read-only inspection is inconclusive, resend only due rows whose workers cannot be observed or confirmed as still working, reuse the same edge_loop_id for any resend, update next_review_at and attempt_count, then stop.
...
Required review:
Check mailbox first for matching edge-loop receipts, results, and result acknowledgements, peek due workers through `<public-entrypoint>->houmao-shared-routines->agent-inspect`, and use active status probes only as the last resort before resending any request.
