...
Resolve live bindings first. Then hand mailbox work to `<public-entrypoint>->houmao-shared-routines->agent-email-comms` for ordinary mailbox operations or `<public-entrypoint>->houmao-shared-routines->process-emails-via-gateway` for one open-mail round. This messaging skill does not restate the lower-level mailbox operation routes.
