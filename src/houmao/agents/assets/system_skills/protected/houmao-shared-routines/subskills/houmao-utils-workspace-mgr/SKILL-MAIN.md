...
- `$<public-entrypoint> utils-workspace-mgr help`
- `$<public-entrypoint> utils-workspace-mgr plan in-repo workspace for <profiles>`
- `$<public-entrypoint> utils-workspace-mgr create the approved workspace plan`
- `$<public-entrypoint> utils-workspace-mgr validate prepared worktrees with <command>`
- `$<public-entrypoint> utils-workspace-mgr summarize workspace <task-name>`

Related skills and boundaries:

- Use `<public-entrypoint>->houmao-shared-routines->agent-definition` for specialist-backed easy launch after workspace preparation.
- Use `<public-entrypoint>->houmao-shared-routines->agent-instance` for broad live-agent lifecycle launch, join, stop, relaunch, or cleanup.
- Use `<public-entrypoint>->houmao-shared-routines->memory-mgr` when the task is only editing an existing live memo rather than preparing memo seed material.
