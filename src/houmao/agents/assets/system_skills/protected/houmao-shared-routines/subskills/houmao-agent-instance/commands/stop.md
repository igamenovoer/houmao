...
Use this action only when the user wants to stop one live managed agent. This remains the canonical general lifecycle stop action even though `<public-entrypoint>->houmao-shared-routines->agent-definition` also owns specialist-scoped easy-instance stop requests.
