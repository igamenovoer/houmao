...
Use `<public-entrypoint>->houmao-shared-routines->process-emails-via-gateway` directly when the task is only one notifier-driven open-mail round.

Use `<public-entrypoint>->houmao-shared-routines->agent-email-comms` directly when the task is only one ordinary list, read, send, reply, mark, move, or archive action.

Use `<public-entrypoint>->houmao-shared-routines->agent-loop-lite` when the user explicitly wants notifier-driven generated skills from Markdown contracts, typed Markdown templates, and direct SQLite state.

Use `<public-entrypoint>->houmao-shared-routines->agent-loop-pro` when the user needs topology authoring, graph decomposition, schema-rich generated execplans, or generated loop run control.
...
- `<public-entrypoint>->houmao-shared-routines->agent-gateway` for gateway attach, discovery, and mail-notifier enable/status/disable.
- `<public-entrypoint>->houmao-shared-routines->process-emails-via-gateway` for the round-oriented open-mail workflow after a notifier prompt supplies the gateway base URL.
- `<public-entrypoint>->houmao-shared-routines->agent-email-comms` for lower-level mailbox status, list, peek, read, send, reply, mark, move, and archive operations.
- `<public-entrypoint>->houmao-shared-routines->agent-messaging` only when the loop also needs explicit prompt or interrupt routing outside ordinary mail.
...
2. Use `<public-entrypoint>->houmao-shared-routines->agent-gateway` to inspect or enable the gateway mail-notifier.
...
4. Send loop mail through `<public-entrypoint>->houmao-shared-routines->agent-email-comms`, generated loop mail tooling, or the gateway `/v1/mail/send|post|reply` surface.
5. Include a short `notify_block` only when the sender needs prominent wake-up guidance in the notifier prompt.
6. When the notifier prompt arrives, use `<public-entrypoint>->houmao-shared-routines->process-emails-via-gateway` for the current open-mail round when available.
