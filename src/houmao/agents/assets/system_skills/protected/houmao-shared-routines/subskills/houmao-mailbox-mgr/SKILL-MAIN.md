...
This protected routine MUST NOT execute standalone. Require an immutable frame from `houmao-admin-entrypoint` or `houmao-agent-entrypoint`; otherwise stop and direct the caller through the matching public entrypoint.
...
- `$<public-entrypoint> mailbox-mgr help`
- `$<public-entrypoint> mailbox-mgr project mailbox status`
- `$<public-entrypoint> mailbox-mgr register <address>`
- `$<public-entrypoint> mailbox-mgr agent binding status for <agent>`

Related skills and boundaries:

- Use `<public-entrypoint>->houmao-shared-routines->agent-email-comms` for ordinary mailbox send, reply, read, mark, move, or archive work.
- Use `<public-entrypoint>->houmao-shared-routines->agent-gateway` for gateway mail-notifier, reminders, or gateway-only state.
- Use `<public-entrypoint>->houmao-shared-routines->agent-definition` for same-root mailbox preparation that is part of specialist-backed easy launch.
