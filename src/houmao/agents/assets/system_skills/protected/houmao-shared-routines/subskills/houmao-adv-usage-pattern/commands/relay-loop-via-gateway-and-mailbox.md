...
Use `<public-entrypoint>->houmao-shared-routines->agent-loop-lite` instead when the user explicitly wants to package a small relay as generated skills with typed Markdown templates and direct SQLite state.

Use `<public-entrypoint>->houmao-shared-routines->agent-loop-pro` instead when the user needs multi-lane routing, fan-out, mixed local-close/relay graph planning, graph policy authoring, a rendered graph, schema-rich generated execplans, or generated loop run-control actions.
...
- `<public-entrypoint>->houmao-shared-routines->agent-inspect` for read-only downstream peeking before due relay handoff resends
...
2. Send the queued gateway prompt to the downstream agent through `<public-entrypoint>->houmao-shared-routines->agent-messaging`.
...
3. if the expected signal is still missing and the handoff is due, use `<public-entrypoint>->houmao-shared-routines->agent-inspect` read-only surfaces to peek the downstream agent for the same `loop_id` and `handoff_id`,
...
Required review:
Check mailbox first for matching relay receipts, results, and result acknowledgements, peek due downstream agents through `<public-entrypoint>->houmao-shared-routines->agent-inspect`, and use active status probes only as the last resort before resending any handoff.
