...
3. Use `<public-entrypoint>->houmao-shared-routines->agent-gateway` to create one or more live reminders through `/v1/reminders`.
...
- Use `<public-entrypoint>->houmao-shared-routines->agent-gateway` for the exact reminder operations instead of restating raw gateway API details here.
