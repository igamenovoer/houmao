...
Use this page when the operator selects `clarify`, asks to clarify an operator-to-agent message before dispatch, or invokes `<public-entrypoint>->houmao-shared-routines->operator-messaging` with an actionable prompt but no subcommand.
