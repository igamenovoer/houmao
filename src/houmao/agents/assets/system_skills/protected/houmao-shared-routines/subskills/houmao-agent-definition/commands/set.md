...
8. If the user asks to mutate env vars or auth files inside the bundle itself, stop and route that request to `<public-entrypoint>->houmao-shared-routines->credential-mgr`.
...
- Do not treat auth-bundle content mutation as a recipe-definition change; use `<public-entrypoint>->houmao-shared-routines->credential-mgr`.
