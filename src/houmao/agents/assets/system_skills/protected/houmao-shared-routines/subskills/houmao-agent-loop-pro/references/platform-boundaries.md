...
- `<public-entrypoint>->houmao-shared-routines->utils-workspace-mgr`: workspace planning, creation, validation, and summaries.
- `<public-entrypoint>->houmao-shared-routines->mailbox-mgr`: mailbox setup, inspection, repair, cleanup, export, registration, and late mailbox binding.
- `<public-entrypoint>->houmao-shared-routines->agent-email-comms`: ordinary mail status, list, read, send, post, reply, mark, move, and archive operations.
- `<public-entrypoint>->houmao-shared-routines->process-emails-via-gateway`: notifier-driven open-mail rounds when the current round provides the gateway base URL.
- `<public-entrypoint>->houmao-shared-routines->agent-messaging`: managed-agent prompt, interrupt, mailbox handoff, and gateway-backed communication routing.
- `<public-entrypoint>->houmao-shared-routines->agent-gateway`: gateway lifecycle, notifier posture, reminders, and gateway posture.
- `<public-entrypoint>->houmao-shared-routines->agent-instance`: managed-agent launch, join, relaunch, stop, and lifecycle control.
- `<public-entrypoint>->houmao-shared-routines->agent-definition`: specialist, project-profile, launch-dossier, credential-defaulting, and pre-launch definition preparation. Treat `houmao-mgr project ...` as its underlying CLI surface for this skill.
- `<public-entrypoint>->houmao-shared-routines->memory-mgr`: memo seed and memory posture.
- `<public-entrypoint>->houmao-shared-routines->agent-inspect`: managed-agent liveness, TUI state, mailbox posture, runtime artifacts, logs, and tmux inspection.
...
- `prepare-workspace` adapts generated workspace contracts, generated agent bindings, and prepared agent/profile facts to `<public-entrypoint>->houmao-shared-routines->utils-workspace-mgr` `plan`, `create`, `validate`, or `summarize`.
...
- Enabling, disabling, or inspecting notifier posture remains owned by `<public-entrypoint>->houmao-shared-routines->agent-gateway`.
- Prompting agents remains owned by `<public-entrypoint>->houmao-shared-routines->agent-messaging`; ordinary mail operations remain owned by `<public-entrypoint>->houmao-shared-routines->agent-email-comms`.
