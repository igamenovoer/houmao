...
- `specs/workspace/`: repository, command, workspace, standard workspace-manager surfaces, validation command contracts, and inputs for `<public-entrypoint>->houmao-shared-routines->utils-workspace-mgr`.
...
The maintained skill boundary is part of the default: `<public-entrypoint>->houmao-shared-routines->mailbox-mgr` owns mailbox administration, `<public-entrypoint>->houmao-shared-routines->agent-email-comms` owns ordinary mail operations, `<public-entrypoint>->houmao-shared-routines->process-emails-via-gateway` owns notifier-driven open-mail rounds, `<public-entrypoint>->houmao-shared-routines->agent-messaging` owns managed-agent communication routing, and `<public-entrypoint>->houmao-shared-routines->agent-gateway` owns gateway lifecycle and posture.
