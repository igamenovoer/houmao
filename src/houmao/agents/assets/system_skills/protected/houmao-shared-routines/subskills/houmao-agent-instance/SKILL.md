...
description: Use Houmao's supported managed-agent lifecycle commands to launch, join, list, stop, relaunch, or clean up live agent instances created from predefined roles, presets, launch dossiers, or specialists.
license: MIT
---
...
- `$<public-entrypoint> agent-instance help`
- `$<public-entrypoint> agent-instance list`
- `$<public-entrypoint> agent-instance launch from specialist <name>`
- `$<public-entrypoint> agent-instance stop <agent>`

Related skills and boundaries:

- Use `<public-entrypoint>->houmao-shared-routines->agent-definition` for specialist-scoped project launch and stop entry points or reusable definition authoring.
- Use `<public-entrypoint>->houmao-shared-routines->agent-inspect` for generic read-only state, logs, artifacts, mailbox posture, or screen inspection.
- Use `<public-entrypoint>->houmao-shared-routines->agent-messaging` for prompt, interrupt, mailbox routing, or reset-context work.
- Use `<public-entrypoint>->houmao-shared-routines->agent-gateway` for gateway sidecar lifecycle and gateway-only control.
...
`<public-entrypoint>->houmao-shared-routines->agent-definition` also owns the specialist-scoped easy `launch` and `stop` entry points, but this skill remains the canonical follow-up lifecycle surface for broader live-agent management.
...
- Use `commands/list.md` only when the user wants the lifecycle-oriented list of current live managed agents. For generic read-only inspection of one agent, use `<public-entrypoint>->houmao-shared-routines->agent-inspect`.
...
- Treat this skill as the canonical follow-up lifecycle surface after any specialist-scoped `launch` or `stop` handled through `<public-entrypoint>->houmao-shared-routines->agent-definition`.
...
- Do not present this skill as the canonical owner of generic managed-agent inspection; use `<public-entrypoint>->houmao-shared-routines->agent-inspect` for that read-only work.
