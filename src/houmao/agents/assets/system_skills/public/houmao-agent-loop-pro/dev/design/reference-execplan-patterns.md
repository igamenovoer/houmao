...
- `specs/workspace/`: repository, command, workspace, standard workspace-manager surfaces, validation command contracts, and inputs for `houmao-shared-routines->houmao-utils-workspace-mgr`.
...
The maintained skill boundary is part of the default: `houmao-shared-routines->houmao-mailbox-mgr` owns mailbox administration, `houmao-shared-routines->houmao-agent-email-comms` owns ordinary mail operations, `houmao-shared-routines->houmao-process-emails-via-gateway` owns notifier-driven open-mail rounds, `houmao-shared-routines->houmao-agent-messaging` owns managed-agent communication routing, and `houmao-shared-routines->houmao-agent-gateway` owns gateway lifecycle and posture.
