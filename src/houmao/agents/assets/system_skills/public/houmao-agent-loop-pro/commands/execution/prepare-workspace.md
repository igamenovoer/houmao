...
8. Use `houmao-shared-routines->houmao-utils-workspace-mgr` for supported workspace planning, creation, validation, or summaries.
...
- Keep standard worktree creation and repair routed through `houmao-shared-routines->houmao-utils-workspace-mgr` when it can represent the layout.
...
- Do not create workspaces by hand when `houmao-shared-routines->houmao-utils-workspace-mgr` can represent the layout.
