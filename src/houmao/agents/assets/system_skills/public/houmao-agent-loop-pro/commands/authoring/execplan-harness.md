...
- include observed notifier posture when available, but route actual notifier enable/disable through `houmao-shared-routines->houmao-agent-gateway`;
- route operator prompts through `houmao-shared-routines->houmao-agent-messaging` and ordinary mail through `houmao-shared-routines->houmao-agent-email-comms`;
