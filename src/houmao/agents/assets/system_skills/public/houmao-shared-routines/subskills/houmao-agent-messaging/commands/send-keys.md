...
4. When the current context does not already confirm live gateway availability, run `agents single ... gateway status` or `agents self gateway status` first.
5. Use `agents single ... gateway send-keys` or `agents self gateway send-keys` for the exact raw key delivery.
6. Use `agents single ... gateway tui state|history` or `agents self gateway tui state|history` when the task needs the exact raw gateway-owned TUI state before or after the key sequence.
...
```text
<chosen houmao-mgr launcher> agents single --agent-name <name> gateway send-keys --sequence "<[Escape]>"
```
...
- Do not redirect raw control-input work to `agents single ... prompt` or `agents self prompt`.
- Do not claim that scoped `agents ... gateway send-keys` appends an implicit trailing `Enter`.
