...
- Credential bundle CRUD, secret mutation, auth-file edits, login flows, and credential rename belong to `houmao-shared-routines->houmao-credential-mgr`.
...
- no registered credentials: add or login one credential through `houmao-shared-routines->houmao-credential-mgr`.
