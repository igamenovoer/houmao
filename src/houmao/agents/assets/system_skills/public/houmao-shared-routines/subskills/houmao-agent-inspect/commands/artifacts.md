...
4. Run `agents single ... state`, `agents self state`, or `GET /houmao/agents/{agent_ref}/state` first to recover stable identity fields such as `manifest_path` and `session_root`.
...
8. For headless sessions, inspect the turn-artifact files under the session root when the user needs raw provider output beyond `agents single ... turn stdout|stderr`.
