...
Use this reference workflow only when the user wants a fresh default Kimi Code OAuth login and wants the resulting Kimi Code home imported into Houmao Kimi credential storage. This is a Kimi-specific login/import workflow, not a maintained `houmao-mgr ... credentials kimi login` helper.

Do not use this workflow for Kimi Platform API keys or compatible provider API keys. Route API-key material through `../commands/add.md` or `../commands/set.md` with `kimi-credential-kinds.md`.
...
4. If the user provided an API key, a `config.toml`, a credential JSON file, or an existing Kimi Code home instead of asking for a fresh default OAuth login, stop and route to `commands/add.md` or `commands/set.md`.
