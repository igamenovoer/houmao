...
The supported multi-step self-notification workflow now lives in the Houmao advanced-usage skill `houmao-shared-routines->houmao-adv-usage-pattern`.
...
Within that pattern, keep using `houmao-shared-routines->houmao-agent-email-comms` only for the ordinary mailbox operations themselves such as `status`, `list`, `peek`, `read`, `send`, `reply`, and `archive`.
