...
description: "Use when standard AG-UI events or already-chosen Houmao implementation payloads must be validated, framed, rendered, published, routed, or interpreted."
...
1. **Handle explicit help first** without running validation, rendering, framing, gateway, or publishing commands.
2. **Validate the inherited actor frame** from `houmao-shared-routines` and resolve the explicit or verified-self gateway target when publishing.
3. **Select one subcommand** from **Subcommands** and preserve the protocol-versus-implementation split.
4. **Resolve one launcher** and validate inputs before framing, rendering, or publishing.
5. **Execute the selected protocol or implementation operation** using the maintained `houmao-mgr ag-ui` surface.
6. **Return the result** with event validation, framing or rendering output, delivery evidence, and live-only publication limits.

If the user's task does not map cleanly to these steps, use the native planning tool to build a step-by-step plan from the AG-UI subcommands, protocol boundary, implementation boundary, gateway contract, and user request, then execute the plan.

## Subcommands

| Subcommand | Use For | Detail |
| --- | --- | --- |
| `validate` | Validate standard AG-UI event shape or an already-chosen Houmao implementation payload. | **Protocol Events** and **Houmao Implementations** |
| `frame` | Frame a validated event batch as JSON, JSON Lines, or SSE. | **Protocol Events** |
| `render` | Render a generic tool call or a validated Houmao implementation payload into standard events. | **Protocol Events** and **Houmao Implementations** |
| `publish` | Deliver a rendered event batch to the current live gateway target. | **Publish to Houmao Gateway** |
| `help` | Read-only protocol, implementation, rendering, publication, and endpoint-boundary guidance. | **Help** |

## Actor Frame Gate

This parent-scoped routine loads only through `houmao-shared-routines`. Require the immutable admin or verified-agent frame validated by that parent. Keep gateway and delivery targets explicit in the admin branch; use freshly verified self gateway context by default in the agent branch. Missing or mismatched frames fail closed.
...
- `$houmao-shared-routines interop-ag-ui help`
- `$houmao-shared-routines interop-ag-ui validate these AG-UI events`
- `$houmao-shared-routines interop-ag-ui render a custom frontend tool call`
- `$houmao-shared-routines interop-ag-ui publish these events to my GUI`
...
- Use `houmao-shared-routines->houmao-agent-messaging` for prompts, interrupts, and live managed-agent communication.
