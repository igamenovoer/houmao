...
- `houmao-shared-routines->houmao-agent-inspect` for read-only downstream worker peeking before due resends
...
2. Send the queued request prompt to the worker through `houmao-shared-routines->houmao-agent-messaging`.
...
3. if the expected signal is still missing and the loop is due, use `houmao-shared-routines->houmao-agent-inspect` to peek the downstream worker for the same `edge_loop_id`,
...
Required review:
Check mailbox first for matching edge-loop receipts, results, and result acknowledgements, peek due workers through `houmao-shared-routines->houmao-agent-inspect`, and use active status probes only as the last resort before resending any request.
