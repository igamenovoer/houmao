...
description: Use when filesystem or project mailbox roots, accounts, registrations, structural state, cleanup, export, or late managed-agent mailbox bindings need administration.
license: MIT
...
This parent-scoped routine loads only through `houmao-shared-routines`. Require the immutable admin or verified-agent frame validated by that parent; otherwise stop before mailbox administration routing.

- Admin branch: require an explicit project, mailbox root, account, or managed-agent binding target before mutation.
- Agent branch: require freshly verified self identity and default only the agent binding target to verified self. Mailbox roots and peer accounts remain explicit.

Keep administration separate from ordinary participation, which routes through `agent-email-comms` under the same public entrypoint.
...
- `$houmao-shared-routines mailbox-mgr help`
- `$houmao-shared-routines mailbox-mgr project mailbox status`
- `$houmao-shared-routines mailbox-mgr register <address>`
- `$houmao-shared-routines mailbox-mgr agent binding status for <agent>`

Related skills and boundaries:

- Use `houmao-shared-routines->houmao-agent-email-comms` for ordinary mailbox send, reply, read, mark, move, or archive work.
- Use `houmao-shared-routines->houmao-agent-gateway` for gateway mail-notifier, reminders, or gateway-only state.
- Use `houmao-shared-routines->houmao-agent-definition` for same-root mailbox preparation that is part of specialist-backed easy launch.
...
- Read [commands/init.md](commands/init.md) to bootstrap or validate one arbitrary filesystem mailbox root or one project mailbox root.
- Read [commands/status.md](commands/status.md) to inspect mailbox-root health for one arbitrary filesystem mailbox root or one project mailbox root.
- Read [commands/register.md](commands/register.md) to register one filesystem mailbox account under one arbitrary mailbox root or one project mailbox root.
- Read [commands/unregister.md](commands/unregister.md) to deactivate or purge one filesystem mailbox account under one arbitrary mailbox root or one project mailbox root.
- Read [commands/repair.md](commands/repair.md) to rebuild filesystem mailbox root index state for one arbitrary mailbox root or one project mailbox root.
- Read [commands/cleanup.md](commands/cleanup.md) to clean inactive or stashed registrations under one arbitrary mailbox root or one project mailbox root.
- Read [commands/clear-messages.md](commands/clear-messages.md) to clear delivered mailbox messages while preserving registrations under one arbitrary mailbox root or one project mailbox root.
- Read [commands/export.md](commands/export.md) to archive all accounts or selected filesystem mailbox accounts under one arbitrary mailbox root or one project mailbox root.
- Read [commands/accounts-list.md](commands/accounts-list.md) to inspect mailbox registrations under one arbitrary mailbox root or one project mailbox root.
- Read [commands/accounts-get.md](commands/accounts-get.md) to inspect one mailbox registration under one arbitrary mailbox root or one project mailbox root.
- Read [commands/messages-list.md](commands/messages-list.md) to inspect structural message projections for one mailbox address under one arbitrary mailbox root or one project mailbox root.
- Read [commands/messages-get.md](commands/messages-get.md) to inspect one structurally projected message for one mailbox address under one arbitrary mailbox root or one project mailbox root.
- Read [commands/messages-clear.md](commands/messages-clear.md) to clear delivered mailbox messages visible to one selected mailbox address while preserving registrations and other accounts.
- Read [commands/agent-binding-status.md](commands/agent-binding-status.md) to inspect late filesystem mailbox posture for one existing local managed agent.
- Read [commands/agent-binding-register.md](commands/agent-binding-register.md) to add or update one late filesystem mailbox binding for one existing local managed agent.
- Read [commands/agent-binding-unregister.md](commands/agent-binding-unregister.md) to remove one late filesystem mailbox binding from one existing local managed agent.
...
- Use `commands/init.md`, `commands/status.md`, `commands/register.md`, `commands/unregister.md`, `commands/repair.md`, `commands/cleanup.md`, `commands/clear-messages.md`, `commands/export.md`, `commands/accounts-list.md`, `commands/accounts-get.md`, `commands/messages-list.md`, `commands/messages-get.md`, or `commands/messages-clear.md` only when the task is mailbox-root administration or structural mailbox inspection.
- Use `commands/agent-binding-status.md`, `commands/agent-binding-register.md`, or `commands/agent-binding-unregister.md` only when the task is late mailbox binding for one existing local managed agent.
...
- Route requests to remove all delivered emails while preserving mailbox accounts to `commands/clear-messages.md`; do not route that request to registration cleanup or account unregister.
- Route requests to remove delivered emails for one selected mailbox account while preserving other accounts to `commands/messages-clear.md`; do not route that request to all-account `clear-messages`, registration cleanup, or account unregister.
- Route requests to archive or export filesystem mailbox state to `commands/export.md`; preserve selected-account scope with repeated `--address` values and expose `--symlink-mode preserve` only when the user explicitly wants symlink preservation.
...
- When the task is attaching mailbox support to an already-running local managed agent, route to `commands/agent-binding-register.md` instead of treating it as generic account registration.
