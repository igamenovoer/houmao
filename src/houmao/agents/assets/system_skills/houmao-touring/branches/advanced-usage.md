...
- Do not restate generated loop templates, topology contracts, run charters, workspace execution rules, routing packets, mailbox result protocol, reminder protocol, memory file layouts, or credential lifecycle details inline; keep those on the selected owning skill.
...
2. Confirm the user's advanced intent:
   ...
   - isolated multi-agent workspace planning or execution,
...
- **Isolated multi-agent workspace management** - use `houmao-utils-workspace-mgr` to plan or execute in-repo or out-of-repo workspaces, per-agent worktrees, local-only shared repos, safe local-state symlinks, launch-profile cwd updates, and optional memo-seed workspace rules.
...
- isolated workspace planning and execution belongs to `houmao-utils-workspace-mgr`,
...
- Do not restate tree-loop or generic-loop plan templates, run charters, stop modes, routing packets, mailbox result protocol, workspace execution details, or reminder protocol inline.
