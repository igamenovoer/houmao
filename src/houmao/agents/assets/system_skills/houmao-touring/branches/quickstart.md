...
When `command -v claude`, `command -v codex`, and `command -v kimi || command -v kimi-code` all fail:

- Explain that the packaged Houmao distribution currently ships tool adapters for `claude`, `codex`, and `kimi`.
