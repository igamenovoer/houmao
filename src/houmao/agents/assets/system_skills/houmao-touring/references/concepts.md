...
- **Tool adapter** — the per-tool contract that tells Houmao how to build and launch one CLI agent family (for example `claude`, `codex`, or `kimi`). Owning skill: `houmao-agent-definition`.
