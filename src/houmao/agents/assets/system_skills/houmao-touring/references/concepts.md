...
- **Loop plan** — a single-file or bundle plan that names a master, participants, and the intended tree-loop or relay topology. The plan is the author-time artifact that the loop skills consume at `start`. Owning skills: `houmao-agent-loop-pairwise`, `houmao-agent-loop-pairwise-v2`, `houmao-agent-loop-pairwise-v3`, `houmao-agent-loop-pairwise-v4`, `houmao-agent-loop-generic`.
