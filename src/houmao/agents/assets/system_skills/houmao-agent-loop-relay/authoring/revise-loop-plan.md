# Revise A Relay Loop Plan

Use this page when a plan already exists, but the user wants to tighten forwarding boundaries, change the designated master or origin, revise completion or stop policy, add or remove scripts, or re-render the final graph.
...
5. Re-validate the route policy. Silence is not authorization.
6. Re-validate the graph semantics:
   - the user agent stays outside the execution loop
   - the master owns the supervision loop
   - forward relay work moves downstream until egress returns the final result to the origin
7. Re-render the Mermaid graph if topology, completion, stop posture, or egress behavior changed.
8. Refresh the normalized run-charter summary if any user-visible control field changed.

## Revision Guardrails

- Do not quietly widen forwarding authority while revising another part of the plan.
- Do not leave a stale graph in place after changing the run topology.
- Do not move final completion evaluation away from the designated master unless the plan explicitly changes the master.
