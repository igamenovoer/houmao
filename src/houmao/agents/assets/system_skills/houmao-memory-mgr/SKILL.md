...
5. Use `--memo-seed-policy initialize|replace|fail-if-nonempty` when the user requests launch-time application behavior. If the user supplies seed content without a policy, rely on the default `initialize` policy. Use `--clear-memo-seed` when the user asks to remove stored seed content. Never combine `--clear-memo-seed` with a seed source or seed policy.
