...
6. Use `agents mail check` when the task is to inspect current unread or recent message state without mutating it.
...
<chosen houmao-mgr launcher> agents mail check --agent-name <name> --unread-only
...
- Do not route mailbox send, reply, post, or mark-read through this inspection action.
