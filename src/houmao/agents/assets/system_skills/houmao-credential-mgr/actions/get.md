...
4. Render the selected command template: `project.credentials.<tool>.get` or `internals.native-agent.credentials.<tool>.get`.
...
<chosen houmao-mgr launcher> --print-json internals command-templates render --id internals.native-agent.credentials.<tool>.get --intent '<json>'
```
