"""Timeline comparison utilities for the explore harness."""
...
from houmao.explore.claude_code_state_tracking.models import TimelineState


@dataclass(frozen=True)
class TimelineComparison:
    """Comparison summary for groundtruth versus replay timelines."""
...
def compare_timelines(
    *,
    groundtruth: list[TimelineState],
    replay: list[TimelineState],
    ...
    markdown = _comparison_markdown(comparison=comparison)
    return comparison, markdown


def _differing_fields(*, ground_item: TimelineState, replay_item: TimelineState) -> list[str]:
...
def _comparison_markdown(*, comparison: TimelineComparison) -> str:
    ...
    lines = [
        "# Claude Code State Tracking Comparison",
        "",
