...
`agents join` and `agents launch` auto-install the control-oriented default sets into managed homes — `mailbox-full`, `agent-memory`, `advanced-usage`, `touring`, `user-control`, `agent-inspect`, `agent-messaging`, and `agent-gateway` as defined in `src/houmao/agents/assets/system_skills/catalog.toml`. Because `user-control` includes `houmao-project-mgr`, `houmao-specialist-mgr`, `houmao-credential-mgr`, `houmao-agent-definition`, `houmao-agent-loop-pairwise`, `houmao-agent-loop-pairwise-v2`, and `houmao-agent-loop-generic`, managed homes gain the project-management front door plus the stable pairwise, enriched pairwise-v2, and generic loop graph-planning skills by default, `houmao-memory-mgr` is available there for memo/pages memory work, `houmao-agent-inspect` is available there as the generic read-only inspection entrypoint, and `houmao-touring` is installed there as a manual-only guided entrypoint. The lifecycle-only `houmao-agent-instance` and utility-only `houmao-utils-llm-wiki` are not managed-home defaults. To prepare an *external* tool home with the broader CLI-default selection, which adds `houmao-agent-instance` but still keeps the utility skill explicit-only, run:

```bash
houmao-mgr system-skills install --tool claude,codex,copilot,gemini
```

When you want a named subset rather than the CLI-default selection, use `--skill-set <name>`, for example `houmao-mgr system-skills install --tool codex --skill-set user-control`. The LLM Wiki utility is explicit-only: install it with `houmao-mgr system-skills install --tool codex --skill-set utils` or `houmao-mgr system-skills install --tool codex --skill houmao-utils-llm-wiki`. When you need an explicit external home, run a single-tool command such as `houmao-mgr system-skills install --tool codex --home ~/.codex`.
