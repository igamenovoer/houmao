...
Houmao ships five loop skills. Choose the one that fits your topology and lifecycle needs:
...
| `houmao-agent-loop-pairwise-v4` | `plan` / `initialize` / `start` / `peek` / `ping` / `pause` / `resume` / `recover_and_continue` / `stop` / `hard-kill` | Pairwise graphs with v3 workspace posture plus strict generated document templates and a source-constraint coverage audit for rich task notes |
