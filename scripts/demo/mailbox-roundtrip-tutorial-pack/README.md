...
- Fresh automatic `start` runs persist `shadow_only` in `<output-root>/control/demo_state.json`, and later `roundtrip` and `stop` commands reuse that persisted mode for the same demo root unless an explicit override is supplied again.
