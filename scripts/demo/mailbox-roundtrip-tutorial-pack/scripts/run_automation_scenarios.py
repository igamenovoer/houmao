...
def _scenario_stepwise_start_roundtrip_verify_stop(
    ...
    _assert((layout.control_dir / "sender_stop.json").is_file(), "stop should emit sender_stop.json")
    _assert((layout.control_dir / "receiver_stop.json").is_file(), "stop should emit receiver_stop.json")
...
def _scenario_incompatible_project_dir(
    ...
    _assert(
        not (HELPERS.build_demo_layout(demo_output_dir=demo_dir).control_dir / "sender_start.json").exists(),
...
def _scenario_partial_failure_cleanup(
    ...
    for artifact_name in (
        ...
        _assert((control_dir / artifact_name).is_file(), f"missing cleanup artifact: {artifact_name}")
    return {
        "demo_output_dir": str(demo_dir.resolve()),
        "checks": {
            "cleanup_mode": bool(stop_result.get("cleanup")),
...
def _scenario_interrupted_run_cleanup(
    ...
    for artifact_name in (
        ...
        _assert((layout.control_dir / artifact_name).is_file(), f"missing cleanup artifact: {artifact_name}")
    return {
        "demo_output_dir": str(demo_dir.resolve()),
        "checks": {
            "cleanup_mode": bool(stop_result.get("cleanup")),
