...
def roundtrip_demo(
    *,
    ...
    if not initial_body_file.is_file() or not reply_instructions_file.is_file():
        raise ValueError(
            "copied message inputs are missing from the demo output directory"
        )
    ...
    _append_chat_event(
        layout,
        kind="send",
        sender=_require_non_empty_string(sender_state.get("mailbox_address"), context="sender.address"),
        ...
        thread_id=_require_non_empty_string(mail_send.get("thread_id"), context="mail_send.thread_id"),
    ...
    _append_chat_event(
        ...
        recipient=_require_non_empty_string(sender_state.get("mailbox_address"), context="sender.address"),
    ...
    *,
    ...
    if not isinstance(reply_parent_message_id, str) or not reply_parent_message_id.strip():
        reply_parent_message_id = extract_message_id(
            _artifact_path(layout, "mail_send")
        )
...
def build_report(
    *,
    ...
    cao_payload = _require_mapping(_read_json(layout.cao_start_path), context=str(layout.cao_start_path))
    ...
    report = {
        ...
        "checks": {
            ...
            "mailbox_reply_thread_matches_send": bool(mailbox_inspection["reply_thread_matches_send"]),
            "mailbox_reply_parent_matches_send": bool(mailbox_inspection["reply_parent_matches_send"]),
            ...
            "chat_log_reply_parent_matches_send": bool(
                chat_log["reply_event_parent_matches_send"]
            ),
        },
    }
