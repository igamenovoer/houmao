...
WORKSPACE_DIR="$WORKSPACE_ROOT/demo_cao_codex_$(date +%Y%m%d_%H%M%S)_$$"
...
ensure_cao_server() {
  ...
  if [[ "$reused_existing_process" -eq 1 && -z "$resolved_pid" ]]; then
    ...
    if ! pixi run python -m houmao.cao.tools.cao_server_launcher start \
      ...
      fail "cao-server launcher retry start failed after ownership mismatch (see $launcher_retry_error_path)"
    fi
  ...
  if [[ "$reused_existing_process" -eq 1 && -z "$resolved_pid" ]]; then
    fail "launcher reused an untracked CAO server at ${CAO_BASE_URL}; stop the external server and retry (home=${CAO_LAUNCHER_HOME_DIR}, profile_store=${CAO_PROFILE_STORE})"
  fi
...
mark_workspace_trusted() {
  ...
  local config_path="$codex_home_path/config.toml"
  if [[ ! -f "$config_path" ]]; then
    fail "missing codex config at $config_path"
  fi
  pixi run python - "$config_path" "$workspace_dir" <<'PY'
import sys
from pathlib import Path
...
log "using allowlisted env names: OPENAI_API_KEY OPENAI_BASE_URL OPENAI_ORG_ID"
log "CAO profile store: $CAO_PROFILE_STORE"
...
if [[ ! -f "$AGENT_DEF_DIR/brains/api-creds/codex/personal-a-default/env/vars.env" ]]; then
  skip "missing credential env file for codex profile personal-a-default"
fi
if [[ ! -f "$AGENT_DEF_DIR/brains/api-creds/codex/personal-a-default/files/auth.json" ]]; then
  skip "missing auth.json for codex profile personal-a-default"
fi
...
CODEX_HOME_PATH="$(extract_json_field "$WORKSPACE_DIR/build.log" home_path)"
mark_workspace_trusted "$CODEX_HOME_PATH" "$WORKSPACE_DIR"
log "marked workspace trusted for Codex: $WORKSPACE_DIR"
...
if [[ -z "${RESPONSE_TEXT// }" ]]; then
  fail "prompt response extraction was empty (source=$RESPONSE_TEXT_SOURCE)"
fi
...
pixi run python - "$REPORT_PATH" "$SESSION_MANIFEST" "$WORKSPACE_DIR" "$RESPONSE_TEXT" "$RESPONSE_TEXT_SOURCE" <<'PY'
import json
import sys
