...
PRECHECK_PATH="$AUTOTEST_CONTROL_DIR/case-real-agent-preflight.preflight.json"
RESULT_PATH="$AUTOTEST_CONTROL_DIR/case-real-agent-preflight.result.json"
...
if pixi run python \
  ...
  write_result_json "$RESULT_PATH" "real-agent-preflight" "passed" "$AUTOTEST_DEMO_OUTPUT_DIR"
else
  if [[ -f "$PRECHECK_PATH" ]]; then
    cp "$PRECHECK_PATH" "$STDOUT_PATH"
  fi
  write_result_json "$RESULT_PATH" "real-agent-preflight" "failed" "$AUTOTEST_DEMO_OUTPUT_DIR" "preflight command failed"
  exit 1
fi
