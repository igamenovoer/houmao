...
print_help() {
  cat <<'EOF'
Usage:
  scripts/demo/gateway-mail-wakeup-demo-pack/run_demo.sh [auto|start|manual-send|manual-send-many|inspect|verify|stop] [--snapshot-report] [--demo-output-dir <path>] [--jobs-dir <path>] [command args...]
EOF
}

while [[ $# -gt 0 ]]; do
  case "$1" in
    auto|start|manual-send|manual-send-many|inspect|verify|stop)
      COMMAND="$1"
      shift
      ;;
    --snapshot-report)
...
DEMO_OUTPUT_DIR="$(resolve_path "$RAW_DEMO_OUTPUT_DIR" "tmp/demo/gateway-mail-wakeup-demo-pack")"
BASE_ARGS=(
  --repo-root "$REPO_ROOT"
  --pack-dir "$SCRIPT_DIR"
...
case "$COMMAND" in
  ...
  verify)
    ...
    "${CMD[@]}"
    ;;
  start|inspect|stop|manual-send|manual-send-many)
