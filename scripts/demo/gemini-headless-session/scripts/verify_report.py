#!/usr/bin/env python3
"""Verify/snapshot gemini headless demo reports."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Verify or snapshot gemini headless demo report")
    ...
    if sanitized.get("backend") != "gemini_headless":
        raise ValueError("backend must be gemini_headless")
    if sanitized.get("tool") != "gemini":
        raise ValueError("tool must be gemini")
