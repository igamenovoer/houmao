...
CASE_ID="parallel-all-phases-auto"
RAW_DEMO_OUTPUT_DIR=""
RAW_EXPECTED_REPORT=""
...
print_help() {
  cat <<'EOF'
Usage:
  scripts/demo/passive-server-parallel-validation-demo-pack/autotest/run_autotest.sh [--case <case-id>] [--demo-output-dir <path>] [--expected-report <path>]
...
case "$CASE_ID" in
  parallel-preflight|parallel-all-phases-auto)
    ;;
  *)
...
if [[ -n "$RAW_DEMO_OUTPUT_DIR" ]]; then
  ...
  DEMO_OUTPUT_DIR="$REPO_ROOT/scripts/demo/passive-server-parallel-validation-demo-pack/outputs/autotest/${CASE_ID}-${STAMP}"
fi
