...
| --- | --- | --- | --- | --- |
| Runtime root and parents | yes | yes | Durable | Needed for generated homes, manifests, sessions, and server roots; defaults to the active project overlay in maintained command flows |
...
| `<runtime-root>/sessions/<backend>/<session-id>/gateway/run/` and launcher/gateway log files | Ephemeral or log-style cleanup after verified stop is usually fine; use `houmao-mgr agents single --agent-id <id> cleanup logs` or `houmao-mgr admin cleanup runtime logs` and do not treat stable gateway files such as `queue.sqlite`, `state.json`, or `events.jsonl` as log scratch. Opt-in gateway diagnostic logs under `gateway/logs/diagnostics/` follow the same cleanup-sensitive log posture as `gateway.log`. |
...
| `<runtime-root>/sessions/<backend>/<session-id>/manifest.json` and stable gateway artifacts | Treat as durable runtime state while the session is active or resumable; `houmao-mgr agents single --agent-id <id> cleanup session` and `houmao-mgr admin cleanup runtime sessions` remove whole stopped envelopes, while log cleanup intentionally leaves durable gateway state behind |
| `<runtime-root>/sessions/<backend>/<session-id>/mailbox-secrets/` | Session-local Stalwart secret material is cleanup-sensitive, not scratch; remove it only after the session is stopped, typically through `houmao-mgr agents single --agent-id <id> cleanup mailbox` |
...
| `<registry-root>/external_agents/<external-agent-id>/record.json` | Registry-owned remote locator metadata for `houmao-mgr agents external`; remove with `agents external remove` rather than local lifecycle cleanup |
