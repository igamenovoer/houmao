...
<active-overlay>/runtime/
  ...
  manifests/
    <home-id>.yaml
  loop-runs/
    pairwise-v2/      # legacy retained runtime-family reference
      <run-id>/
        record.json
        events.jsonl
  mailbox-credentials/
...
| Path pattern | Created by | Later written by | Purpose | Contract level | Cleanup notes |
| --- | --- | --- | --- | --- | --- |
| `<runtime-root>/loop-runs/pairwise-v2/<run-id>/` | legacy accepted pairwise-v2 `start` guidance | legacy pairwise-v2 recovery and stop guidance | Runtime-owned envelope for one logical legacy run across participant restarts | Legacy stable path family | Do not delete while the run may still be inspected or recovered |
| `<runtime-root>/loop-runs/pairwise-v2/<run-id>/record.json` | legacy accepted pairwise-v2 `start` guidance | legacy pairwise-v2 recovery and stop guidance | Latest durable recovery contract for one accepted logical legacy run, including `run_id`, `recovery_epoch`, plan reference and freshness marker, participants, durable page references, mailbox bindings, declarative wakeup posture, and recoverable-versus-terminal state | Legacy stable operator-facing artifact | Treat as durable runtime state rather than scratch |
| `<runtime-root>/loop-runs/pairwise-v2/<run-id>/events.jsonl` | legacy accepted pairwise-v2 `start` guidance | legacy pairwise-v2 recovery and stop guidance | Append-only history of acceptance, recovery attempts, stop transitions, and terminal hard-kill transitions | Legacy stable path, append-only payload | Safe to inspect; do not treat it as the authoritative latest record |
...
| `<session-root>/gateway/desired-config.json` | gateway-capability publication | attach/detach lifecycle | Desired host/port reuse hints, execution mode, TUI tracking timing, and optional diagnostic logging settings for later gateway starts | Stable operator-facing artifact | Durable |
...
- current-session `houmao-mgr agents self gateway attach` still remains invalid until the same logical session is discoverable through the shared registry or maintained pair authority
...
- `houmao-mgr agents self join` writes the same `<session-root>/manifest.json`, placeholder `agent_def/`, placeholder `brain_manifest.json`, session-local `gateway/` subtree, and per-agent memory metadata that native launches expect.
...
- Shared-registry publication for joined sessions uses a long sentinel lease instead of a short renewable lease because `agents self join` is a one-shot adoption command rather than a resident launcher.
...
- `<runtime-root>/loop-runs/pairwise-v2/<run-id>/record.json` is the durable runtime-owned recovery record for one accepted logical legacy pairwise-v2 run.
- `<runtime-root>/loop-runs/pairwise-v2/<run-id>/events.jsonl` is append-only legacy recovery history, not the authoritative latest record.
