# Brain Launch Runtime `send-keys`
...
The local tmux 3.4 man page documents a broader key vocabulary than the subset currently guaranteed by `gig-agents`.

Important distinction:

- tmux accepts more special key names than the runtime currently allows
- `gig-agents` currently guarantees only the exact keys listed in the previous section
