...
- Runtime-owned tmux-backed sessions publish one secret-free live record per canonical agent name.
...
- `agent key`: The full lowercase SHA-256 hex digest of the canonical agent name, used as the live-agent directory name.
...
- a standalone packaged JSON Schema is a follow-up change, not a shipped contract yet.
