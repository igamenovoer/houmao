...
- Stored easy-profile memo seeds apply before prompt composition and provider startup. Direct `project easy instance launch --specialist ...` launches do not apply one because no reusable easy profile was selected, and there is no one-shot memo-seed override flag on the launch surface.
