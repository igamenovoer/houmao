...
- Live attach and request execution currently support runtime-owned `local_interactive` sessions and runtime-owned native headless sessions (`claude_headless`, `codex_headless`, `gemini_headless`, `kimi_headless`). Legacy REST-backed manifest records may still be recognized for old-artifact inspection or explicit rejection, but new public launches do not create them.
