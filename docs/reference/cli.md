...
- `houmao-mgr agents gateway mail-notifier status|enable|disable ...` expose the live gateway notifier control surface with the same target-resolution rules; `enable --appendix-text TEXT` updates the runtime prompt appendix, omitted preserves it, and `--appendix-text ""` clears it
