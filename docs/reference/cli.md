...
For actor-pack lifecycle outside managed launch or join, use `houmao-mgr system-skills list|install|status|upgrade|uninstall ...`. `--home` is optional for single-target lifecycle commands: omitted `--home` resolves from the tool-native home env var first and otherwise falls back to the project-scoped default home for tool targets, while `--tool universal` resolves to `~/.agents`. External install and upgrade default to the `admin` pack; repeat `--pack admin|agent` for an explicit selection. Uninstall removes only selected receipt-owned packs, or every owned pack when `--pack` is omitted. That surface is documented in [system-skills](cli/system-skills.md).
