...
4. `houmao-mgr system-skills ...` installs the current Houmao-owned mailbox skill sets into resolved Claude, Codex, Kimi Code, Gemini, or Copilot homes, or into the `universal` `~/.agents/skills` target, when you need those skills outside a Houmao-managed launch or join flow.
