...
```mermaid
flowchart LR
    subgraph EASY["Easy lane (project ...)"]
...
- **Easy lane** uses the noun `profile`, is specialist-backed, and exposes a smaller, opinionated authoring surface. It is the right place to start when you want reusable defaults without answering every low-level launch question. CLI: `houmao-mgr project profile ...`.
- **Explicit lane** uses the noun `launch-profile`, is recipe-backed, and exposes the fuller low-level launch contract. It is the right place to be when you need precise control over the underlying source recipe and the full launch field set. CLI: `houmao-mgr internals native-agent launch-dossiers ...`.

A specialist-backed project profile and a recipe-backed explicit launch profile are stored as the same kind of catalog object. The difference is the source lane (`specialist` vs `recipe`) and the profile lane (`easy_profile` vs `launch_profile`) recorded on each entry. Both lanes project into the same compatibility tree under `.houmao/agents/launch-profiles/<name>.yaml`.

That shared storage and shared projection path do not make the command families interchangeable. Manage project profiles through `houmao-mgr project profile ...` and native launch dossiers through `houmao-mgr internals native-agent launch-dossiers ...`; wrong-lane management attempts fail with guidance to the correct lane instead of reading, mutating, or deleting the other lane's profile.
...
| **recipe** | explicit | yes | `presets/<name>.yaml` | operator (explicit) | The reusable low-level source definition. The CLI surface is `internals native-agent recipes ...`; `project agents presets ...` is a compatibility alias for the same files. |
| **project profile** | easy | yes | `launch-profiles/<name>.yaml` | operator (easy) | Specialist-backed reusable birth-time launch configuration. Targets exactly one specialist. |
...
- a relaunch-only provider chat-session policy for future selected-agent `agents single ... relaunch` operations,
...
flowchart TD
    ...
    C["3. Launch-profile defaults<br/>(project profile or explicit launch profile)"]
...
- `--reuse-home` is also override-only. `houmao-mgr project agents launch --reuse-home` reuses one compatible preserved home for the resolved managed identity, rebuilds current Houmao-managed launch inputs onto that home, and still creates fresh live session authority for the new launch. The flag is not stored back into the profile.
- Launch-time force takeover is also override-only. `--force [keep-stale|clean]` applies to the current `project agents launch` invocation, never persists into the stored profile, and never changes what the next launch from that profile will request by default.
...
houmao-mgr internals native-agent launch-dossiers set --name researcher --clear-system-skills
```
...
This policy applies only to future instances launched from the profile. It does not mutate already-running agents, recipes, specialists, or the provider's own chat store. A direct `houmao-mgr agents single --agent-name <name> relaunch --chat-session-mode ...` override wins for that one relaunch and does not rewrite the stored profile.
...
The stored appendix becomes the default runtime guidance used later when that session's gateway mail-notifier renders a wake-up prompt. Live notifier edits are runtime-owned: `houmao-mgr agents single --agent-name <name> gateway mail-notifier enable --appendix-text ...` or `houmao-mgr agents self gateway mail-notifier enable --appendix-text ...` can replace or clear the running session's appendix, but it never rewrites the stored launch profile.

Both profile lanes expose the same stored default:

- Easy lane: `houmao-mgr project profile create|set --gateway-mail-notifier-appendix-text <text>`.
- Explicit lane: `houmao-mgr internals native-agent launch-dossiers add|set --gateway-mail-notifier-appendix-text <text>`.
...
Memo seeds are applied **before** prompt composition and provider startup. That means the managed prompt header, prompt overlay, and provider bootstrap all see the already-seeded `houmao-memo.md` path and page tree. Launch-time direct overrides such as `--name`, `--auth`, `--workdir`, or prompt appendix flags do not rewrite the stored memo seed, and direct `project agents launch --specialist ...` launches do not apply one because no reusable project profile was selected.
...
3. one-shot launch appendix append when `project agents launch` supplies `--append-system-prompt-text` or `--append-system-prompt-file`,
...
- `houmao-mgr project agents list` and `houmao-mgr project agents get` report the originating project-profile identity when runtime-backed state makes it resolvable, and continue to report the originating specialist when available.
- `houmao-mgr agents single --agent-name <name> state` and `houmao-mgr agents global list` report the same lane and profile information for explicit profile-backed managed agents.
- Inspection output never includes secret credential values inline; auth is reported by display name only.

## Picking A Lane

Use the easy lane (`project specialist` plus `project profile`) when:
...
Use the explicit lane (`internals native-agent recipes` plus `internals native-agent launch-dossiers`) when:
...
Direct launch-time overrides and stored profile edits are separate. `project agents launch ... --workdir <path>` affects only that launch; it does not rewrite the reusable profile.

Use the patch command for ordinary stored-default edits:

```bash
# Easy lane
houmao-mgr project profile set --name <profile> --workdir /repos/next-target

# Explicit lane
houmao-mgr internals native-agent launch-dossiers set --name <profile> --workdir /repos/next-target
```
...
houmao-mgr project profile create --name <profile> --specialist <specialist> --yes

# Explicit lane replacement
houmao-mgr internals native-agent launch-dossiers add --name <profile> --recipe <recipe> --yes
```

Replacement is lane-bounded and clears omitted optional fields, including any previously stored managed system-skill policy, gateway mail-notifier appendix, or memo seed. An project-profile replacement cannot replace an explicit recipe-backed launch profile with the same name, and an explicit launch-profile replacement cannot replace an project profile.
...
```bash
# Easy lane
houmao-mgr project specialist create --name <name> --tool <tool> ...
houmao-mgr project profile create --name <profile> --specialist <name> ...
houmao-mgr project profile set --name <profile> ...
houmao-mgr project agents launch --profile <profile>           # project-profile-backed
houmao-mgr project agents launch --profile <profile> --reuse-home
houmao-mgr project agents launch --specialist <name> --name <managed-name>

# Explicit lane
houmao-mgr internals native-agent recipes add --name <recipe> --role <role> --tool <tool> ...
houmao-mgr internals native-agent launch-dossiers add --name <profile> --recipe <recipe> ...
```

`--profile` and `--specialist` cannot be combined on `project agents launch`. Root/global/single `agents launch` paths are not maintained public launch paths.

Use `--reuse-home` when you want fresh launch inputs plus preserved provider-local home state. Use `agents single ... relaunch` when you want the same built session/home posture restarted without rebuilding. `--reuse-home` never persists into the reusable profile itself.

`project agents presets ...` remains valid as a compatibility alias for the same files that `internals native-agent recipes ...` administers; both names map to `.houmao/agents/presets/<name>.yaml`.

For full option tables and edge cases, see the [`houmao-mgr` CLI reference](../reference/cli/houmao-mgr.md).

## See Also

- [Easy Specialists](easy-specialists.md) — operator workflow for the easy lane (specialist → optional project profile → instance).
...
- [`houmao-mgr` CLI reference](../reference/cli/houmao-mgr.md) — authoritative option tables for `project profile`, `project agents launch`, and `internals native-agent launch-dossiers`.
