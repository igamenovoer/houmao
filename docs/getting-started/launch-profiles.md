...
- `--reuse-home` is also override-only. `houmao-mgr agents launch --reuse-home` and `houmao-mgr project agents launch --reuse-home` reuse one compatible preserved home for the resolved managed identity, rebuild current Houmao-managed launch inputs onto that home, and still create fresh live session authority for the new launch. The flag is not stored back into the profile.
- Launch-time force takeover is also override-only. `--force [keep-stale|clean]` applies to the current `agents launch` or `project agents launch` invocation, never persists into the stored profile, and never changes what the next launch from that profile will request by default.
...
3. one-shot launch appendix append when `agents launch` or `project agents launch` supplies `--append-system-prompt-text` or `--append-system-prompt-file`,
...
Direct launch-time overrides and stored profile edits are separate. `project agents launch ... --workdir <path>` or `agents launch --launch-profile <profile> --workdir <path>` affects only that launch; it does not rewrite the reusable profile.
...
`--profile` and `--specialist` cannot be combined on `project agents launch`. `--launch-profile` and `--agents` cannot be combined on `agents launch`.
...
- [`houmao-mgr` CLI reference](../reference/cli/houmao-mgr.md) — authoritative option tables for `project profile`, `internals native-agent launch-dossiers`, and `agents launch --launch-profile`.
