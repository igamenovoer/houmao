...
| `houmao-agent-loop-pairwise-v3` | Extend pairwise-v2 with an authored workspace contract and memo-first prestart: author a master-owned pairwise loop plan from user intent, choose `standard` or `custom` workspace mode, keep standard in-repo posture task-scoped under `houmao-ws/<task-name>/...`, optionally launch missing participants from provided launch profiles during `initialize`, require email/mailbox support for every required participant, verify or enable gateway mail-notifier behavior for required mail-driven participants with supported live gateway and mailbox surfaces, write per-agent run guidance into memo during `initialize`, and use a compact master-only `start` trigger that says to read memo and begin, delivered by mail by default. | Routes through the same execution skills as pairwise-v2, plus `houmao-utils-workspace-mgr` when the plan chooses a Houmao-standard workspace; the skill itself is authoring plus `plan|initialize|start|peek|ping|pause|resume|recover_and_continue|stop|hard-kill` control |
