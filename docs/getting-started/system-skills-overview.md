...
When authoring or validating plans with `houmao-agent-loop-pairwise-v2`, `houmao-agent-loop-pairwise-v3`, or `houmao-agent-loop-generic`, the `houmao-mgr internals graph high` commands provide deterministic structural helpers that can be called from inside an agent session:
