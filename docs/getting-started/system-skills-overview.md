...
| `houmao-agent-loop-pairwise-v3` | Extend pairwise-v2 with an authored workspace contract: author a master-owned pairwise loop plan from user intent, choose `standard` or `custom` workspace mode, keep standard in-repo posture task-scoped under `houmao-ws/<task-name>/...`, and operate the accepted run through the same enriched lifecycle as v2. | Routes through the same execution skills as pairwise-v2, plus `houmao-utils-workspace-mgr` when the plan chooses a Houmao-standard workspace; the skill itself is authoring plus `plan|initialize|start|peek|ping|pause|resume|recover_and_continue|stop|hard-kill` control |
