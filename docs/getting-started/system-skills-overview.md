...
| `houmao-agent-loop-pairwise-v4` | Extend pairwise-v3 with template-driven source-contract authoring: preserve policy-bearing source constraints, fill strict generated document templates for `plan.md`, participant notes, reporting templates, and bookkeeping templates, and finalize a constraint coverage audit before treating a rich generated bundle as complete. | Routes through the same execution skills as pairwise-v3; the skill itself is authoring plus `plan|initialize|start|peek|ping|pause|resume|recover_and_continue|stop|hard-kill` control with generated document-template guidance |
