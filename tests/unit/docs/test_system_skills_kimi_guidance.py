...
def test_credential_manager_documents_kimi_crud_without_login_helper() -> None:
    ...
    assert "supported CRUD tools: `claude`, `codex`, `gemini`, `kimi`" in skill_text
    ...
    assert "`tool`: one of `claude`, `codex`, `gemini`, or `kimi`" in add_text
    assert "`tool`: one of `claude`, `codex`, `gemini`, or `kimi`" in set_text
...
def test_packaged_guidance_includes_kimi_touring_and_avoids_stale_headless_claims() -> None:
    ...
    assert "tool adapters for `claude`, `codex`, `gemini`, and `kimi`" in quickstart_text
    assert "`claude`, `codex`, `gemini`, or `kimi`" in concepts_text
