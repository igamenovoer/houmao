...
def _create_project_specialist(runner: CliRunner, *, repo_root: Path, tmp_path: Path) -> None:
    ...
    assert Path(create_payload["metadata_path"]).is_file()
    assert (
...
def _install_fake_tmux_runtime(
    ...
    monkeypatch.setattr(
        ...
        lambda *, session_name: TmuxBackedAuthorityHealth(
            state="healthy",
            session_exists=bool(session_name),
...
def test_houmao_mgr_agents_launch_supports_registry_first_local_control(
    ...
    monkeypatch.setattr(
        "houmao.srv_ctrl.commands.managed_agents.tmux_session_exists",
        lambda *, session_name: session_name.startswith("HOUMAO-gpu-"),
    )
...
def test_houmao_mgr_agents_launch_supports_registry_first_local_interactive_control(
    ...
    assert "attach_command    tmux attach-session -t HOUMAO-gpu-" in launch_result.output
...
def test_houmao_mgr_agents_launch_supports_explicit_launch_profiles(
    ...
    monkeypatch.setattr(
        "houmao.srv_ctrl.commands.managed_agents.tmux_session_exists",
        lambda *, session_name: session_name.startswith("HOUMAO-alice"),
    )
    ...
    payload = {
        ...
        "mailbox": {
            ...
            "filesystem": {
                ...
                "mailbox_path": "/tmp/mailbox/mailboxes/HOUMAO-alpha@agents.localhost",
                "inbox_path": "/tmp/mailbox/mailboxes/HOUMAO-alpha@agents.localhost/inbox",
                "sqlite_path": "/tmp/mailbox/index.sqlite",
                "local_sqlite_path": (
                    "/tmp/mailbox/mailboxes/HOUMAO-alpha@agents.localhost/mailbox.sqlite"
                ),
            },
        },
        "gateway": {
...
def test_houmao_mgr_help_excludes_retired_server_group() -> None:
    runner = CliRunner()

    help_result = runner.invoke(cli, ["--print-json", "--help"])
    assert help_result.exit_code == 0, help_result.output
    assert "server" not in cli.commands
    assert "server" not in help_result.output

    server_result = runner.invoke(cli, ["--print-json", "server", "--help"])
