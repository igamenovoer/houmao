...
you want to project a real local login-state file into the runtime home.
Launches without `OPENAI_API_KEY` still require a non-empty top-level JSON
object in the local ignored `files/auth.json`.
...
For Claude credential profiles, include any local state/bootstrap files needed
for runtime `.claude.json` materialization, but keep them untracked.
